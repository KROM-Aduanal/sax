Imports gsol.monitoreo
Imports gsol.BaseDatos
Imports gsol.Componentes.SistemaBase
Imports System.Windows.Forms
Imports System.Reflection
Imports gsol.BaseDatos.Operaciones
Imports gsol.seguridad
Imports System.Configuration
Imports System.Collections.Generic
Imports System.Drawing
Imports System.Globalization
Imports System.Text
Imports Wma.ExceptionsC
Imports gsol.krom

'Imports Gsol.BaseDatos.Operaciones.IOperacionesCatalogo
Imports System.ComponentModel
Imports System.Data
Imports gsol.basededatos
Imports gsol.documento

Imports gsol.correoelectronico
Imports System.IO
Imports Wma.Exceptions
'Imports Wma.Exceptions.IExceptions

Namespace gsol

    Public Class Organismo

#Region "Enums"

        Public Enum VerificarTipoDatoDBNULL

            Numero
            Cadena

        End Enum

        Public Enum Modulos
            SinDefinir = 0
            Paises = 1
            Personas = 2
            EstadosPaises
            Ciudades
            Acciones
            Funciones
            Idiomas
            GirosEmpresariales
            GruposEmpresariales
            DivisionesEmpresariales
            Usuarios
            Domicilios
            Modulos
            Aplicaciones
            Entidades
            GestionEntidades
            GestionAtributos
            Permisos
            Empresas
            MisEmpresas
            Clientes
            Proveedores
            Prospectos
            GestionEmpresasGeneral
            Ejecutivos
            AplicacionesModulos
            GestionNegocioPotencial
            CaracteristicasComercializacion
            GruposVenta
            Cotizaciones
            ProductosServicios
            ProductosServiciosDivision
            NotasCotizacion
            GestionDetalleCotizacion
            GestionCaracteristicasCotizacion
            CaracteristicasCotizacion
            Contactos
            Impuestos
            GruposImpuestos
            GestionDetalleKardex
            GestionProductosComercializacion
            GestionEtapasComercializacion
            GestionCaracteristicasComercializacion
            KardexEmpresarial
            GestionContactos
            EquipoComputo
            CuentasContables
            GestionCuentasBancariasEntidad
            CuentasBancariasEntidad
            ConceptosPagosHechos
            CatBeneficiarios
            TipoCambio
            Bancos
            ConceptoTransaccionPoliza
            SolicitudPago
            GestionCartera
            Cartera
            ControlCFDI
            MaestroOperaciones
            UnidadesNegocio
            ControlDocumentos
            Beneficiarios
            ConceptosPago
            Rutas
            AgentesCarga
            AduanaSeccion
            Naves
            Puertos
        End Enum

        Public Enum NivelContrasena
            baja
            media
            alta
        End Enum

#End Region

#Region "Atributos"

        Private _configuracionGlobal As Configuracion

        Private _bitacora As IBitacoras '= ObtenerInstancia("IBitacoras.dll", "gsol.IBitacoras")
        'Private _bitacora As Object

        Private _conexionsingleton As Object ' SingletonSQLServer

        Private _ventanadialogo As GsDialogo

        Private _ileerarchivo As ILeerArchivo

        Private _icifrar As ICifrado

        Private _ensamblados As List(Of String)

        Private _tokens As Dictionary(Of String, String)

        'ManifiestoGlobal via singleton de la configuración central del CMF, por consecuencia para el ERP
        ' Private _configuracion As LeerArchivoXML32

        'Watcher para todo el CMF por consecuencia para todo el ERP
        Private _tagWatcher As TagWatcher

        'R para todo el sistema CMF por consecuencia para todo el ERP
        Private _irecursosSistema As IRecursosSistemas.RecursosCMF

#End Region

#Region "Constructores"

        Sub New()

            _bitacora = New BitacoraCapaDatos

            _conexionsingleton = SingletonSQLServer.ObtenerInstancia()

            _ileerarchivo = New LeerArchivoXML32

            _icifrar = New Cifrado256

            _ensamblados = New List(Of String)

            _tokens = New Dictionary(Of String, String)

            _tagWatcher = New TagWatcher

            'Carga de constantes del archivo encriptado

            '_configuracionGlobal = New Configuracion

        End Sub

#End Region

#Region "Propiedades"

        ReadOnly Property R As IRecursosSistemas.RecursosCMF

            Get

                Return _irecursosSistema

            End Get

        End Property

        ReadOnly Property Watcher As TagWatcher

            Get

                Return _tagWatcher

            End Get

        End Property

        ReadOnly Property ManifiestoGlobal(ByVal token_ As Configuracion.DatosGlobalesSistema) As String

            Get
                'If _configuracionGlobal Is Nothing Then
                '    _configuracionGlobal = New Configuracion
                'End If

                'Return Configuracion.ConstanteGlobal(token_)

                Configuracion.ObtenerInstancia()

                Return Configuracion.ConstanteGlobal(token_)

            End Get

        End Property

        Public ReadOnly Property Ensamblados As IList

            Get

                Return CargaRecursosEnsamblados()

            End Get

        End Property

        Public ReadOnly Property Tokens As Dictionary(Of String, String)

            Get

                Return CargaRecursosTokens()

            End Get

        End Property

        Public Property Cifrar As ICifrado
            Get
                Return _icifrar
            End Get
            Set(ByVal value As ICifrado)
                _icifrar = value
            End Set
        End Property

        Public Property ConexionSingleton As SingletonSQLServer

            Get
                'Obtiene instancia singleton
                ObtenerInstanciaSingleton()

                Return _conexionsingleton

            End Get

            Set(ByVal value As SingletonSQLServer)

                _conexionsingleton = value

            End Set
        End Property

#End Region

#Region "Methods"

        Private Function ObtenerInstancia(
            ByVal nombreEnsamblado_ As String,
            ByVal tipoControl_ As String,
            ByVal bloque_ As String
        ) As Object
            Dim control_ As Object = Nothing

            'CARGA EL EMSAMBLADO
            Dim ensamblado_ As Assembly =
                Assembly.LoadFrom("..\..\..\..\..\..\masters\vb\esp\sax2.0\bin\Core\" & bloque_ & "\" & nombreEnsamblado_)


            'OBTIENE EL TIPO DE CONTROL A GENERAR
            Dim tipo_ As Type = ensamblado_.GetType(tipoControl_)

            'OBTIENE UNA INSTANCIA DEL CONTROL
            control_ = Activator.CreateInstance(tipo_)

            Return control_

        End Function

        Public Function GetEnumDescription(ByVal EnumConstant As [Enum]) As String 'Implements GetEnumDescription
            Dim fi As FieldInfo = EnumConstant.GetType().GetField(EnumConstant.ToString())
            Dim attr() As DescriptionAttribute =
                          DirectCast(fi.GetCustomAttributes(GetType(DescriptionAttribute),
                          False), DescriptionAttribute())

            If attr.Length > 0 Then
                Return attr(0).Description
            Else
                Return EnumConstant.ToString()
            End If

        End Function

        Public Sub PreparaListaDeplegable(ByRef combobox_ As ComboBox,
                                  ByVal vistaiuconfiguracion_ As String,
                                  ByVal vistapresentacion_ As String,
                                  ByVal campoparamostrar_ As String,
                                  ByVal campodevalor_ As String,
                                  Optional ByVal clausulalibre_ As String = Nothing)

            Dim ioperacioneslistadesplegable_ As IOperacionesCatalogo

            Dim valor_ As String = "DESC"

            ioperacioneslistadesplegable_ = New OperacionesCatalogo


            ioperacioneslistadesplegable_.OrdenarResultados(0) = 0

            ioperacioneslistadesplegable_.VistaEncabezados = vistaiuconfiguracion_

            ioperacioneslistadesplegable_.OperadorCatalogoConsulta = vistapresentacion_

            ioperacioneslistadesplegable_.CantidadVisibleRegistros = 500

            ioperacioneslistadesplegable_.ClausulasLibres = clausulalibre_


            ioperacioneslistadesplegable_.GenerarVista()

            If ioperacioneslistadesplegable_.Vista.Tables.Count > 0 Then

                combobox_.DataSource = ioperacioneslistadesplegable_.Vista.Tables(0)

                combobox_.DisplayMember = campoparamostrar_

                combobox_.ValueMember = campodevalor_

            Else

                combobox_.Items.Clear()

                combobox_.Items.Add("<Vacío>")

            End If

        End Sub


        Public Sub PreparaListaDeplegable(ByRef combobox_ As ComboBox,
                                          ByVal vistaiuconfiguracion_ As String,
                                          ByVal vistapresentacion_ As String,
                                          ByVal campoparamostrar_ As String,
                                          ByVal campodevalor_ As String,
                                          ByVal clausulalibre_ As String,
                                          ByVal columnaOrdenar_ As Int32,
                                          ByVal tipoOrdenamiento_ As Int32)

            Dim limiteRegistros_ As Int32 = 500

            Dim ioperacioneslistadesplegable_ As IOperacionesCatalogo

            Dim valor_ As String = "DESC"

            ioperacioneslistadesplegable_ = New OperacionesCatalogo

            Select Case tipoOrdenamiento_

                Case 0 : valor_ = "DESC"
                Case 1 : valor_ = "ASC"
                Case 2 : valor_ = "INDEFINIDO"

            End Select

            ioperacioneslistadesplegable_.OrdenarResultados(columnaOrdenar_) = tipoOrdenamiento_

            ioperacioneslistadesplegable_.VistaEncabezados = vistaiuconfiguracion_

            ioperacioneslistadesplegable_.OperadorCatalogoConsulta = vistapresentacion_

            ioperacioneslistadesplegable_.CantidadVisibleRegistros = limiteRegistros_

            ioperacioneslistadesplegable_.ClausulasLibres = clausulalibre_

            ioperacioneslistadesplegable_.GenerarVista()

            If ioperacioneslistadesplegable_.Vista.Tables.Count > 0 Then

                combobox_.DataSource = ioperacioneslistadesplegable_.Vista.Tables(0)

                combobox_.DisplayMember = campoparamostrar_

                combobox_.ValueMember = campodevalor_

            Else
                combobox_.Items.Clear()

                combobox_.Items.Add("<Vacío>")

            End If

        End Sub

        Public Sub PreparaListaDeplegable(ByRef combobox_ As ComboBox,
                                          ByVal vistaiuconfiguracion_ As String,
                                          ByVal vistapresentacion_ As String,
                                          ByVal campoparamostrar_ As String,
                                          ByVal campodevalor_ As String,
                                          ByVal clausulalibre_ As String,
                                          ByVal columnaOrdenar_ As Int32,
                                          ByVal tipoOrdenamiento_ As Int32,
                                          ByVal limiteRegistros_ As Int32)

            Dim ioperacioneslistadesplegable_ As IOperacionesCatalogo

            Dim valor_ As String = "DESC"

            ioperacioneslistadesplegable_ = New OperacionesCatalogo

            Select Case tipoOrdenamiento_

                Case 0 : valor_ = "DESC"
                Case 1 : valor_ = "ASC"
                Case 2 : valor_ = "INDEFINIDO"

            End Select

            ioperacioneslistadesplegable_.OrdenarResultados(columnaOrdenar_) = tipoOrdenamiento_

            ioperacioneslistadesplegable_.VistaEncabezados = vistaiuconfiguracion_

            ioperacioneslistadesplegable_.OperadorCatalogoConsulta = vistapresentacion_

            ioperacioneslistadesplegable_.CantidadVisibleRegistros = limiteRegistros_

            ioperacioneslistadesplegable_.ClausulasLibres = clausulalibre_

            ioperacioneslistadesplegable_.GenerarVista()

            If ioperacioneslistadesplegable_.Vista.Tables.Count > 0 Then

                combobox_.DataSource = ioperacioneslistadesplegable_.Vista.Tables(0)

                combobox_.DisplayMember = campoparamostrar_

                combobox_.ValueMember = campodevalor_

            Else
                combobox_.Items.Clear()

                combobox_.Items.Add("<Vacío>")

            End If

        End Sub

        Private Sub CargarDropDown(ByVal ComboBox_ As ComboBox,
                           ByVal Ve_ As String,
                           ByVal VT_ As String,
                           ByVal CampoClave_ As String,
                           ByVal Campodescripcion_ As String,
                           ByVal Validacion_ As String)

            Dim _dtAux As DataTable = New DataTable

            If ComboBox_.DataSource Is Nothing Then

                PreparaListaDeplegable(ComboBox_, Ve_, VT_, Campodescripcion_, CampoClave_, Validacion_)

                _dtAux = DirectCast(ComboBox_.DataSource, DataTable)

                If _dtAux.DataSet.Tables.Count > 0 Then

                    ComboBox_.DataSource = _dtAux

                    ComboBox_.DisplayMember = Campodescripcion_

                    ComboBox_.ValueMember = CampoClave_

                End If

            Else

                ComboBox_.DataSource = _dtAux

                ComboBox_.DisplayMember = Campodescripcion_

                ComboBox_.ValueMember = CampoClave_

            End If

        End Sub

        Public Function ValidarVacios(ByVal valor_ As Object,
                                       ByVal tipodato_ As VerificarTipoDatoDBNULL) As Object

            If IsDBNull(valor_) Then

                Select Case tipodato_

                    Case VerificarTipoDatoDBNULL.Numero

                        valor_ = 0

                    Case VerificarTipoDatoDBNULL.Cadena

                        valor_ = ""

                End Select

            End If

            Return valor_

        End Function



        Public Function ObtenerInstanciaSingleton() As SingletonSQLServer

            _conexionsingleton = SingletonSQLServer.ObtenerInstancia()

        End Function

        'Para un sólo paquete de datos
        Public Sub TraduceDatos(ByRef ioperaciones_ As IOperacionesCatalogo,
                                     ByVal estructuraDatos_ As IEntidadDatos,
                                     ByVal tipoOperacion_ As IOperacionesCatalogo.TiposOperacionSQL)

            ioperaciones_.VisualizacionCamposConfigurada = IOperacionesCatalogo.TiposVisualizacionCampos.PresentarVisibles

            'PENDIENTE
            ioperaciones_.EstructuraConsulta = estructuraDatos_

            Select Case tipoOperacion_

                Case IOperacionesCatalogo.TiposOperacionSQL.Modificar

                    ioperaciones_.KeyValues = estructuraDatos_.UpdateOnKeyValues

                Case IOperacionesCatalogo.TiposOperacionSQL.Eliminar

                    ioperaciones_.KeyValues = estructuraDatos_.UpdateOnKeyValues

            End Select

            'ioperaciones_.CadenaEncabezados = Nothing
            'Flush
            For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.Visible = ICaracteristica.TiposVisible.No

            Next

            For Each campo_ As CampoVirtual In estructuraDatos_.Atributos

                For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                    If caracteristica_.Nombre = campo_.Atributo.ToString Then

                        Select Case tipoOperacion_

                            Case IOperacionesCatalogo.TiposOperacionSQL.Insercion

                                'Convertimos el valor en opcional
                                caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.Opcional

                                'Asignamos el valor de la estructura
                                caracteristica_.ValorAsignado = campo_.Valor


                            Case IOperacionesCatalogo.TiposOperacionSQL.Modificar

                                'Convertimos el valor en opcional
                                caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.Opcional

                                'Asignamos el valor de la estructura
                                caracteristica_.ValorAsignado = campo_.Valor

                            Case IOperacionesCatalogo.TiposOperacionSQL.Consulta

                                caracteristica_.Visible = ICaracteristica.TiposVisible.Si

                        End Select

                    End If

                Next

            Next

        End Sub

        'Para un bulk de datos
        Public Sub TraduceDatos(ByRef ioperaciones_ As IOperacionesCatalogo,
                                     ByVal bulkDatos_ As List(Of IEntidadDatos),
                                     ByVal tipoOperacion_ As IOperacionesCatalogo.TiposOperacionSQL)

            ioperaciones_.VisualizacionCamposConfigurada = IOperacionesCatalogo.TiposVisualizacionCampos.PresentarVisibles

            'PENDIENTE
            ioperaciones_.EstructuraConsulta = bulkDatos_(0)

            'ioperaciones_.CadenaEncabezados = Nothing
            'Flush
            For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.Visible = ICaracteristica.TiposVisible.No

            Next

            'Leemos la batería del bulk para trducirla a registros RegustroIOperaciones
            For Each entidadDatos_ As IEntidadDatos In bulkDatos_

                'Definimos un nuevo registro donde colocaremos los campos antes de añadirlo en IOperaciones.Bulk
                Dim registroNuevo_ As New RegistroIOperaciones

                'For Each campo_ As CampoVirtual In bulkDatos_(0).Atributos
                For Each campo_ As CampoVirtual In entidadDatos_.Atributos

                    Dim indiceCampo_ As Int32 = 0

                    For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                        Dim campoNuevo_ As ICaracteristica = New CaracteristicaCatalogo

                        'Asignamos los valores actuales que tenga la caracteristica del apuntador
                        campoNuevo_ = caracteristica_.Clone

                        If campoNuevo_.Nombre = campo_.Atributo.ToString Then

                            Select Case tipoOperacion_

                                Case IOperacionesCatalogo.TiposOperacionSQL.Insercion

                                    'Convertimos el valor en opcional
                                    campoNuevo_.PuedeInsertar = ICaracteristica.TiposRigorDatos.Opcional

                                    'Asignamos el valor de la estructura
                                    campoNuevo_.ValorAsignado = campo_.Valor

                                    caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.Opcional

                                    'Entramando un nuevo registro

                                    'registroNuevo_.Add(indiceCampo_) = campoNuevo_
                                    registroNuevo_.Add(campoNuevo_)

                                    indiceCampo_ += 1


                                Case IOperacionesCatalogo.TiposOperacionSQL.Modificar

                                    'Convertimos el valor en opcional
                                    campoNuevo_.PuedeModificar = ICaracteristica.TiposRigorDatos.Opcional

                                    'Asignamos el valor de la estructura
                                    campoNuevo_.ValorAsignado = campo_.Valor

                                    caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.Opcional
                                    'Entramando un nuevo registro

                                    'registroNuevo_.Add(indiceCampo_) = campoNuevo_
                                    registroNuevo_.Add(campoNuevo_)

                                    indiceCampo_ += 1

                                Case IOperacionesCatalogo.TiposOperacionSQL.Consulta

                                    campoNuevo_.Visible = ICaracteristica.TiposVisible.Si

                            End Select

                        End If

                    Next

                Next

                'sólo para consultas MOP 17/02/2021
                ioperaciones_.AgregaRegistroBulk(registroNuevo_)

            Next

        End Sub

        Public Sub OptimizaOperacion(ByRef ioperaciones_ As IOperacionesCatalogo,
                                     ByVal estructuraConsulta_ As IEntidadDatos,
                                     ByVal tipoOperacion_ As IOperacionesCatalogo.TiposOperacionSQL)

            ioperaciones_.VisualizacionCamposConfigurada = IOperacionesCatalogo.TiposVisualizacionCampos.PresentarVisibles

            'PENDIENTE
            ioperaciones_.EstructuraConsulta = estructuraConsulta_

            'ioperaciones_.CadenaEncabezados = Nothing
            'Flush
            For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.Visible = ICaracteristica.TiposVisible.No

            Next


            For Each campo_ As CampoVirtual In estructuraConsulta_.Atributos

                'valoresCampos_ = valoresCampos_ & ", [" & campo_.Atributo.ToString & "='" & campo_.Valor & "']"

                For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                    If caracteristica_.Nombre = campo_.Atributo.ToString Then

                        Select Case tipoOperacion_

                            Case IOperacionesCatalogo.TiposOperacionSQL.Insercion

                                'NO IMPLEMENTADO

                            Case IOperacionesCatalogo.TiposOperacionSQL.Modificar

                                'NO IMPLEMENTADO

                            Case IOperacionesCatalogo.TiposOperacionSQL.Consulta

                                caracteristica_.Visible = ICaracteristica.TiposVisible.Si

                        End Select

                    End If

                Next

            Next

        End Sub

        Public Sub OptimizaOperacion(ByRef ioperaciones_ As IOperacionesCatalogo,
                                     ByVal CamposPermitidos_ As List(Of String),
                                     ByVal tipoOperacion_ As IOperacionesCatalogo.TiposOperacionSQL)

            'Flush
            For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.No

                caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.No

            Next


            For Each campo_ As String In CamposPermitidos_

                For Each caracteristica_ As ICaracteristica In ioperaciones_.Caracteristicas.Values

                    If caracteristica_.Nombre = campo_ Then

                        Select Case tipoOperacion_

                            Case IOperacionesCatalogo.TiposOperacionSQL.Insercion

                                caracteristica_.PuedeInsertar = ICaracteristica.TiposRigorDatos.Si

                            Case IOperacionesCatalogo.TiposOperacionSQL.Modificar

                                caracteristica_.PuedeModificar = ICaracteristica.TiposRigorDatos.Si

                            Case IOperacionesCatalogo.TiposOperacionSQL.Consulta

                                caracteristica_.Visible = ICaracteristica.TiposVisible.Si

                        End Select

                    End If

                Next

            Next

        End Sub

        Public Function TieneResultados(ByVal buscadorDatos_ As IOperacionesCatalogo) As Boolean

            If Not buscadorDatos_ Is Nothing Then

                With buscadorDatos_

                    If Not .Vista.Tables Is Nothing Then

                        If Not .Vista.Tables.Count <= 0 Then

                            If Not .Vista.Tables(0).Rows Is Nothing Then

                                If Not .Vista.Tables(0).Rows.Count <= 0 Then

                                    Return True

                                End If

                            End If

                        End If

                    End If

                End With

            End If

            Return False

        End Function

        Public Sub ConsultaModuloDirecto(ByRef ioperacionesCatalogo_ As IOperacionesCatalogo,
                                              ByVal tokenModulo_ As String,
                                              ByVal estructuraEsperada_ As IEntidadDatos)

            With ioperacionesCatalogo_

                If Not tokenModulo_ Is Nothing And
                    Not .EspacioTrabajo Is Nothing Then

                    EnsamblaModulo(ioperacionesCatalogo_, tokenModulo_)

                    .PreparaCatalogo()

                    .CadenaEncabezados = Nothing

                    .CantidadVisibleRegistros = .CantidadVisibleRegistros

                    OptimizaOperacion(ioperacionesCatalogo_,
                                      estructuraEsperada_,
                                      IOperacionesCatalogo.TiposOperacionSQL.Consulta)


                    .GenerarVista()

                End If

            End With

        End Sub

        Public Function ConsultaModulo(ByVal ioperacionesCatalogo_ As IOperacionesCatalogo,
                                       ByVal tokenModulo_ As String,
                                       ByVal estructuraEsperada_ As IEntidadDatos) As IOperacionesCatalogo

            Dim buscadorDatos_ As IOperacionesCatalogo = New OperacionesCatalogo

            'MsgBox("basta!")

            If Not tokenModulo_ Is Nothing And
                Not ioperacionesCatalogo_.EspacioTrabajo Is Nothing Then

                Try 'Intenta ensamblar la configuración y clonarla

                    buscadorDatos_ = EnsamblaModulo(tokenModulo_).Clone

                    With buscadorDatos_

                        .EspacioTrabajo = ioperacionesCatalogo_.EspacioTrabajo

                        .TipoEscritura = IOperacionesCatalogo.TiposEscritura.Inmediata

                        .ClausulasLibres = ioperacionesCatalogo_.ClausulasLibres

                        .CantidadVisibleRegistros = ioperacionesCatalogo_.CantidadVisibleRegistros

                        .ModalidadConsulta = ioperacionesCatalogo_.ModalidadConsulta 'Conectividad

                        'Busqueda por enlace
                        .TipoInstrumentacion = ioperacionesCatalogo_.TipoInstrumentacion

                        'Estableciendo que el consumo de datos es vía extranet
                        .IDAplicacion = ioperacionesCatalogo_.IDAplicacion

                        .IDRecursoSolicitante = ioperacionesCatalogo_.IDRecursoSolicitante

                        .IDUsuario = ioperacionesCatalogo_.IDUsuario

                        .ClaveDivisionMiEmpresa = ioperacionesCatalogo_.ClaveDivisionMiEmpresa

                        '.OrdenarResultados(-1) = IOperacionesCatalogo.OrdenConsulta.INDEFINIDO

                        .PreparaCatalogo()


                    End With

                    ioperacionesCatalogo_.CadenaEncabezados = Nothing

                    buscadorDatos_.CantidadVisibleRegistros = ioperacionesCatalogo_.CantidadVisibleRegistros

                    OptimizaOperacion(buscadorDatos_,
                                      estructuraEsperada_,
                                      IOperacionesCatalogo.TiposOperacionSQL.Consulta)


                    buscadorDatos_.GenerarVista()

                    If TieneResultados(buscadorDatos_) Then

                        Return buscadorDatos_

                    Else

                        Return Nothing

                    End If

                Catch ex As Exception

                    Return Nothing

                End Try

            End If

            Return Nothing

        End Function

        Public Function ConsultaModulo(ByVal ioperacionesCatalogo_ As IOperacionesCatalogo,
                                       ByVal tokenModulo_ As String) As IOperacionesCatalogo

            Dim buscadorDatos_ As IOperacionesCatalogo = New OperacionesCatalogo

            If Not tokenModulo_ Is Nothing And
                Not ioperacionesCatalogo_.EspacioTrabajo Is Nothing Then

                Try 'Intenta ensamblar la configuración y clonarla

                    buscadorDatos_ = EnsamblaModulo(tokenModulo_).Clone

                    With buscadorDatos_

                        .EspacioTrabajo = ioperacionesCatalogo_.EspacioTrabajo

                        .TipoEscritura = IOperacionesCatalogo.TiposEscritura.Inmediata

                        .ClausulasLibres = ioperacionesCatalogo_.ClausulasLibres

                        .CantidadVisibleRegistros = ioperacionesCatalogo_.CantidadVisibleRegistros

                        'Establecemos la modalidad para consultar
                        .ModalidadConsulta = ioperacionesCatalogo_.ModalidadConsulta

                        'Busqueda por enlace
                        .TipoInstrumentacion = ioperacionesCatalogo_.TipoInstrumentacion

                        'Estableciendo que el consumo de datos es vía extranet
                        .IDAplicacion = ioperacionesCatalogo_.IDAplicacion

                        .IDRecursoSolicitante = ioperacionesCatalogo_.IDRecursoSolicitante

                        .IDUsuario = ioperacionesCatalogo_.IDUsuario

                        .ClaveDivisionMiEmpresa = ioperacionesCatalogo_.ClaveDivisionMiEmpresa

                        .GenerarVista()

                    End With

                    If TieneResultados(buscadorDatos_) Then

                        Return buscadorDatos_

                    Else

                        Return Nothing

                    End If

                Catch ex As Exception

                    Return Nothing

                End Try

            End If

            Return Nothing

        End Function

        Public Function ConsultaModulo(ByVal ioperacionesCatalogo_ As IOperacionesCatalogo,
                                       ByVal tokenModulo_ As String,
                                       Optional ByVal modalidadConsulta_ As IOperacionesCatalogo.ModalidadesConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir
                        ) As IOperacionesCatalogo

            Dim buscadorDatos_ As IOperacionesCatalogo = New OperacionesCatalogo

            If Not tokenModulo_ Is Nothing And
                Not ioperacionesCatalogo_.EspacioTrabajo Is Nothing Then

                Try 'Intenta ensamblar la configuración y clonarla

                    buscadorDatos_ = EnsamblaModulo(tokenModulo_).Clone

                    With buscadorDatos_

                        .EspacioTrabajo = ioperacionesCatalogo_.EspacioTrabajo

                        .TipoEscritura = IOperacionesCatalogo.TiposEscritura.Inmediata

                        .ClausulasLibres = ioperacionesCatalogo_.ClausulasLibres

                        .ModalidadConsulta = modalidadConsulta_

                        .CantidadVisibleRegistros = ioperacionesCatalogo_.CantidadVisibleRegistros


                        'Establecemos la modalidad para consultar
                        '.ModalidadConsulta = ioperacionesCatalogo_.ModalidadConsulta

                        'Busqueda por enlace
                        .TipoInstrumentacion = ioperacionesCatalogo_.TipoInstrumentacion

                        'Estableciendo que el consumo de datos es vía extranet
                        .IDAplicacion = ioperacionesCatalogo_.IDAplicacion

                        .IDRecursoSolicitante = ioperacionesCatalogo_.IDRecursoSolicitante

                        .IDUsuario = ioperacionesCatalogo_.IDUsuario

                        .ClaveDivisionMiEmpresa = ioperacionesCatalogo_.ClaveDivisionMiEmpresa


                        '.OrdenarResultados(-1) = IOperacionesCatalogo.OrdenConsulta.INDEFINIDO

                        .GenerarVista()

                    End With

                    If TieneResultados(buscadorDatos_) Then

                        Return buscadorDatos_

                    Else

                        Return Nothing

                    End If

                Catch ex As Exception

                    Return Nothing

                End Try

            End If

            Return Nothing

        End Function

        Public Function ConsultaModulo(ByVal espacioTrabajo_ As IEspacioTrabajo,
                                       ByVal tokenModulo_ As String,
                                       Optional ByVal clausulasLibres_ As String = Nothing) As IOperacionesCatalogo

            Dim buscadorDatos_ As IOperacionesCatalogo = New OperacionesCatalogo

            If Not tokenModulo_ Is Nothing And
                Not espacioTrabajo_ Is Nothing Then

                buscadorDatos_ = EnsamblaModulo(tokenModulo_).Clone

                With buscadorDatos_

                    .EspacioTrabajo = espacioTrabajo_

                    .TipoEscritura = IOperacionesCatalogo.TiposEscritura.Inmediata

                    .ClausulasLibres = clausulasLibres_


                    'Establecemos la modalidad para consultar
                    .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.Singleton

                    'Busqueda por enlace
                    .TipoInstrumentacion = IBitacoras.TiposInstrumentacion.GestorIOperaciones

                    'Estableciendo que el consumo de datos es vía extranet
                    .IDAplicacion = IBitacoras.ClaveTiposAplicacion.AplicacionEscritorioKrombase

                    .IDRecursoSolicitante = 0

                    .IDUsuario = 0

                    .ClaveDivisionMiEmpresa = 0


                    .GenerarVista()

                End With

                If TieneResultados(buscadorDatos_) Then

                    Return buscadorDatos_

                Else

                    Return Nothing

                End If

            End If

            Return Nothing

        End Function

        'Deprecated
        Private Function CargaRecursosEnsamblados() As IList

            If _ensamblados.Count >= 1 Then

                Return _ensamblados

            Else

                If _ileerarchivo.StatusArchivo = ILeerArchivo.StatusArchivos.Cargado Then

                    Dim _par As KeyValuePair(Of String, String)

                    For Each _par In _ileerarchivo.Propiedades

                        If _par.Key.Length >= 17 Then

                            If _par.Key.Substring(0, 16) = "NombreEnsamblado" Then

                                _ensamblados.Add(_par.Value)

                            End If

                        End If

                    Next

                Else

                    '_ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml"

                    _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

                    Dim _par As KeyValuePair(Of String, String)

                    Dim listaUnica_ As New List(Of String)

                    For Each _par In _ileerarchivo.Propiedades


                        If _par.Key.Length >= 17 Then

                            If _par.Key.Substring(0, 16) = "NombreEnsamblado" Then

                                If Not listaUnica_.Contains(_par.Value) Then
                                    _ensamblados.Add(_par.Value)

                                    listaUnica_.Add(_par.Value)

                                End If

                            End If

                        End If

                    Next

                End If

                Return _ensamblados

            End If

        End Function

        Private Function CargaRecursosTokens() As Dictionary(Of String, String)

            If _tokens.Count >= 1 Then

                Return _tokens

            Else

                If _ileerarchivo.StatusArchivo = ILeerArchivo.StatusArchivos.Cargado Then

                    Dim par_ As KeyValuePair(Of String, String)

                    For Each par_ In _ileerarchivo.Propiedades

                        _tokens.Add(par_.Key, par_.Value)

                    Next

                Else

                    '_ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "\Configuracion\ConfiguracionGeneralModulos.xml"

                    _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

                    Dim par_ As KeyValuePair(Of String, String)

                    Dim listaUnica_ As New List(Of String)

                    For Each par_ In _ileerarchivo.Propiedades

                        If Not listaUnica_.Contains(par_.Key) Then

                            _tokens.Add(par_.Key, par_.Value)

                            listaUnica_.Add(par_.Key)

                        End If

                    Next

                End If

                Return _tokens

            End If

        End Function

        Public Function Eval(ByVal Expresion As String,
                             Optional ByVal entityName_ As String = Nothing,
                          Optional ByVal ListOfResources_ As List(Of String) = Nothing) As Object

            Dim vbcp As New Microsoft.VisualBasic.VBCodeProvider
            Dim vbc As System.CodeDom.Compiler.ICodeCompiler = vbcp.CreateCompiler
            Dim cpar As New System.CodeDom.Compiler.CompilerParameters
            Dim res As System.CodeDom.Compiler.CompilerResults

            cpar.GenerateExecutable = False ' Generar DLL 
            cpar.GenerateInMemory = True ' Generar en memoria 
            cpar.IncludeDebugInformation = True

            ' Agregar referencias 
            cpar.ReferencedAssemblies.Add("Microsoft.VisualBasic.dll")
            cpar.ReferencedAssemblies.Add("gsol.krom.CampoVirtual64.0.0.0.0.dll")

            cpar.ReferencedAssemblies.Add("gsol.krom.IEnlaceDatos.0.0.0.0.dll")
            cpar.ReferencedAssemblies.Add("gsol.krom.EnlaceDatos64.0.0.0.0.dll")

            cpar.ReferencedAssemblies.Add("gsol.krom.IEntidadDatos.0.0.0.0.dll")
            cpar.ReferencedAssemblies.Add("gsol.krom.EntidadDatos64.0.0.0.0.dll")
            'cpar.ReferencedAssemblies.Add("gsol.krom.CatResponsables64.0.0.0.0.dll")

            If Not entityName_ Is Nothing Then

                'listaLibrerias_.Add("gsol.krom.Responsables64.0.0.0.0.dll")

                cpar.ReferencedAssemblies.Add("gsol.krom." & entityName_ & "64.0.0.0.0.dll")

            End If

            If Not ListOfResources_ Is Nothing Then
                For Each resource_ As String In ListOfResources_
                    cpar.ReferencedAssemblies.Add(resource_)
                Next
            End If


            '        cpar.ReferencedAssemblies.Add("gsol.krom.Referencia64.0.0.0.0.dll")
            'cpar.ReferencedAssemblies.Add("gsol.krom.Mercancias64.0.0.0.0.dll")

            'gsol.krom.CampoVirtual64.0.0.0.0.dll
            'gsol.krom.IEnlaceDatos.0.0.0.0.dll

            ' Compilar 
            '"Imports Gsol.krom.IEnlaceDatos" & vbCrLf & _

            'Dim dimension_ As String = "algo"
            'Dim campos_ As String = "campos"

            '        "   Inherits EntidadDatos" & vbCrLf & _
            '"  Enum " & dimension_ & vbCrLf & _
            '"    " & campos_ & vbCrLf & _
            '"  End Enum" & vbCrLf & _
            '"        Sub New()" & vbCrLf & _
            '"            Dimension = IEnlaceDatos.TiposDimension.Fracciones " & vbCrLf & _
            '"        End Sub" & vbCrLf & _

            '            "Imports Gsol.krom.Referencia.Vt022ConsultaOperaciones" & vbCrLf & _
            '   "Imports Gsol.krom.CatResponsables.Vt026Responsables" & vbCrLf & _

            '"Imports Gsol.krom.CatResponsables.Vt026Responsables" & vbCrLf & _

            res = vbc.CompileAssemblyFromSource(cpar,
                "Imports Microsoft.VisualBasic" & vbCrLf &
                "Imports Gsol.krom" & vbCrLf &
                "Namespace StandartEntities" & vbCrLf &
                " Public Class MyKind" & vbCrLf &
                "  Public Shared Function Eval() As Object " & vbCrLf &
                "   Return " & Expresion & vbCrLf &
                "  End Function" & vbCrLf &
                " End Class" & vbCrLf &
                "End Namespace")


            If res.Errors.Count = 0 Then

                ' Obtengo el Type de la clase recien compilada 
                Dim miclase As System.Type
                miclase = res.CompiledAssembly.GetType("StandartEntities.MyKind")


                ' Obtengo el metodo Eval de la clase 
                Dim funceval As System.Reflection.MethodInfo
                funceval = miclase.GetMethod("Eval")


                ' Ejecuto la funcion Eval recien creada 
                Return funceval.Invoke(Nothing, Nothing)


            Else
                Return res.Errors(0).ErrorText

            End If


        End Function

        'Actualizar
        Public Function ComandosSingletonSQL(ByVal transaccionSQL_ As String,
                                     Optional ByVal respuesta_ As String = Nothing) As TagWatcher



            Dim valorRecuperado_ As String = Nothing



            Dim tagWatcher_ As New TagWatcher



            Try



                With ConexionSingleton.SQLServerSingletonConexion



                    .DataSetReciente.Tables.Clear()



                    tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_000_3900)





                    If .EjecutaConsulta(transaccionSQL_) Then



                        tagWatcher_.SetOK()



                        If Not .DataSetReciente Is Nothing Then



                            If Not .DataSetReciente.Tables Is Nothing Then



                                If .DataSetReciente.Tables.Count >= 1 Then



                                    If Not respuesta_ Is Nothing Then



                                        valorRecuperado_ = .DataSetReciente.Tables(0).Rows(0)(respuesta_).ToString



                                        tagWatcher_.SetOK(.DataSetReciente.Tables(0).Rows.Count, valorRecuperado_)



                                    Else



                                        tagWatcher_.SetOK(.DataSetReciente.Tables(0).Rows.Count)



                                    End If



                                    tagWatcher_.ObjectReturned = .DataSetReciente.Tables(0)



                                End If



                            End If



                        Else

                            tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_000_3901)

                        End If



                    End If



                End With



            Catch exception_ As System.Exception



                tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_000_3901, exception_.Message)



            End Try



            Return tagWatcher_



        End Function

        Public Function ComandosSingletonSQLEspecial(ByVal transaccionSQL_ As String,
                                     Optional ByVal respuesta_ As String = Nothing,
                                     Optional ByVal utilizaBitacora_ As Boolean = True,
                                     Optional ByVal controlador_ As IConexiones.Controladores = IConexiones.Controladores.SQLServer2008) As TagWatcher

            Dim valorRecuperado_ As String = Nothing

            Dim tagWatcher_ As New TagWatcher

            Try

                With ConexionSingleton.SQLServerSingletonConexion

                    .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.Singleton

                    .DataSetReciente.Tables.Clear()

                    tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_000_3900)

                    Select Case utilizaBitacora_

                        Case True

                            If .EjecutaConsulta(transaccionSQL_) Then

                                tagWatcher_.SetOK()

                                If Not .DataSetReciente Is Nothing Then

                                    If Not .DataSetReciente.Tables Is Nothing Then

                                        If .DataSetReciente.Tables.Count >= 1 Then

                                            If Not respuesta_ Is Nothing Then

                                                valorRecuperado_ = .DataSetReciente.Tables(0).Rows(0)(respuesta_).ToString

                                                tagWatcher_.SetOK(.DataSetReciente.Tables(0).Rows.Count, valorRecuperado_)

                                            Else

                                                tagWatcher_.SetOK(.DataSetReciente.Tables(0).Rows.Count)

                                            End If

                                            tagWatcher_.ObjectReturned = .DataSetReciente.Tables(0)

                                        End If

                                    End If

                                Else

                                    tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_000_3901)

                                End If

                            End If

                        Case False
                            '  EjecutaConsultaDirectaEstandarizada()
                            Dim tw_ As New TagWatcher

                            If controlador_ = IConexiones.Controladores.MySQL51 Then

                                .ControladorBaseDatos = IConexiones.Controladores.MySQL51

                                '.ObjetoDatos = IConexiones.TipoConexion.MySQLCommand

                            ElseIf controlador_ = IConexiones.Controladores.SQLServer2008 Then

                                .ControladorBaseDatos = IConexiones.Controladores.SQLServer2008

                                '.ObjetoDatos = IConexiones.TipoConexion.SqlCommand

                            End If

                            tw_ = .EjecutaConsultaDirectaEstandarizadaOpenCloseDataTable(transaccionSQL_)

                            If tw_.Status = TagWatcher.TypeStatus.Ok Then

                                If Not tw_.ObjectReturned Is Nothing Then

                                    tagWatcher_.SetOK()

                                    If Not respuesta_ Is Nothing Then

                                        valorRecuperado_ = tw_.ObjectReturned.Rows(0)(respuesta_).ToString

                                        tagWatcher_.SetOK(tw_.ObjectReturned.Rows.Count, valorRecuperado_)

                                    Else

                                        tagWatcher_.SetOK(tw_.ObjectReturned.Rows.Count)

                                    End If

                                    tagWatcher_.ObjectReturned = tw_.ObjectReturned

                                    Return tw_
                                Else
                                    tagWatcher_.SetError()

                                    Return tw_

                                End If

                            End If
                    End Select



                End With

            Catch exception_ As System.Exception

                tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_000_3901,
                                     exception_.Message)

            End Try

            Return tagWatcher_

        End Function

        Public Function EnsamblaModulo(ByRef ioperacionesModulo_ As IOperacionesCatalogo,
                                       ByVal modulo_ As String) As IOperacionesCatalogo

            Dim tagWatcher_ As New TagWatcher

            Dim ejecuta_ As Boolean = False

            If ioperacionesModulo_ Is Nothing Then

                tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_001_3006)

                Return Nothing

            End If

            'If System.IO.File.Exists(System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "\Configuracion\ConfiguracionGeneralModulos.xml") Then
            'If System.IO.File.Exists(System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml") Then

            '    ' _ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "\Configuracion\ConfiguracionGeneralModulos.xml"
            '    _ileerarchivo.RutaArchivo = Environment.GetEnvironmentVariable("saxpath") & "..\settings\ConfiguracionGeneralModulos.xml"

            '    ejecuta_ = True

            'Else

            '    If System.IO.File.Exists("C:\svn\SVN QA\Configuracion\ConfiguracionGeneralModulos.xml") Then
            '        'Lo buscamos en la ruta por defecto
            '        _ileerarchivo.RutaArchivo = "C:\svn\SVN QA\Configuracion\ConfiguracionGeneralModulos.xml"

            '        ejecuta_ = True

            '    End If

            'End If

            ejecuta_ = True

            With ioperacionesModulo_

                If ejecuta_ Then

                    _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

                    .TablaEdicion = _ileerarchivo.RegresaValor("TablaEdicion." & modulo_)

                    .IdentificadorCatalogo = _ileerarchivo.RegresaValor("CampoLlave." & modulo_)

                    .OperadorCatalogoConsulta = _ileerarchivo.RegresaValor("VT." & modulo_)

                    .VistaEncabezados = _ileerarchivo.RegresaValor("VE." & modulo_)

                    .Nombre = _ileerarchivo.RegresaValor("Descripcion." & modulo_)

                    .NombreToken = modulo_

                    .IdentificadorEmpresa = "-1"

                    Try

                        .IdentificadorEmpresa = _ileerarchivo.RegresaValor("Multiempresa." & modulo_)

                        .OperadorCatalogoBorrado = _ileerarchivo.RegresaValor("OperadorBorrado." & modulo_)

                        .OperadorCatalogoInsercion = _ileerarchivo.RegresaValor("OperadorInserciones." & modulo_)

                        .OperadorCatalogoModificacion = _ileerarchivo.RegresaValor("OperadorModificaciones." & modulo_)

                        .Version = _ileerarchivo.RegresaValor("Version." & modulo_)

                        .NombreClaveUpsert = _ileerarchivo.RegresaValor("NombreClaveUpsert." & modulo_)

                        .Granularidad = _ileerarchivo.RegresaValor("Granularidad." & modulo_)

                        .Entidad = _ileerarchivo.RegresaValor("Entidad." & modulo_)

                        .Dimension = _ileerarchivo.RegresaValor("Dimension." & modulo_)

                        'Valida la condiguración de la propiedad ReflejarEn, si es que viene como propiedad o del XML
                        If .ReflejarEn = IConexiones.Controladores.SinDefinir Then

                            If _ileerarchivo.Propiedades.ContainsKey("ReflejarEn." & modulo_) Then

                                Dim registroReflejarEn_ As String = _ileerarchivo.RegresaValor("ReflejarEn." & modulo_)

                                .ReflejarEn = DirectCast([Enum].Parse(GetType(IEnlaceDatos.DestinosParaReplicacion), registroReflejarEn_), IEnlaceDatos.DestinosParaReplicacion)

                                '.ReflejarEn = _ileerarchivo.RegresaValor("ReflejarEn." & modulo_)

                            End If

                        End If

                        If .OrigenDatos = IConexiones.Controladores.SinDefinir Then

                            If _ileerarchivo.Propiedades.ContainsKey("OrigenDatos." & modulo_) Then

                                Dim valor_ As String = _ileerarchivo.RegresaValor("OrigenDatos." & modulo_)

                                If Not valor_ Is Nothing Then

                                    valor_ = LCase(Trim(valor_))

                                    Select Case valor_

                                        Case "mongodb", "mongo"

                                            .OrigenDatos = IConexiones.Controladores.MongoDB
                                            .ObjetoDatos = IConexiones.TiposRepositorio.DataSetObject
                                            .TipoConexion = IConexiones.TipoConexion.DirectMongoDB
                                            .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.ConexionLibre

                                        Case "sqlserver", "mssql"


                                            .OrigenDatos = IConexiones.Controladores.SQLServer2008
                                            .ObjetoDatos = IConexiones.TiposRepositorio.Automatico
                                            .TipoConexion = IConexiones.TipoConexion.Automatico
                                            .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                                    End Select

                                End If

                            Else

                                'De no existir una configuración, se pondrá por defecto SQL Server solamente en singleton

                                .OrigenDatos = IConexiones.Controladores.SQLServer2008

                                .ObjetoDatos = IConexiones.TiposRepositorio.Automatico

                                .TipoConexion = IConexiones.TipoConexion.Automatico

                                .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                            End If

                        End If

                        If .OrigenDatosAlternativo = IConexiones.Controladores.SinDefinir Then

                            If _ileerarchivo.Propiedades.ContainsKey("OrigenDatosAlternativo." & modulo_) Then

                                Dim valor_ As String = _ileerarchivo.RegresaValor("OrigenDatosAlternativo." & modulo_)

                                If Not valor_ Is Nothing Then

                                    valor_ = LCase(Trim(valor_))

                                    Select Case valor_

                                        Case "mongodb", "mongo"

                                            .OrigenDatosAlternativo = IConexiones.Controladores.MongoDB
                                            .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject
                                            .TipoConexionAlternativa = IConexiones.TipoConexion.DirectMongoDB
                                            .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.ConexionLibre

                                        Case "sqlserver", "mssql"

                                            .OrigenDatosAlternativo = IConexiones.Controladores.SQLServer2008
                                            .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject
                                            .TipoConexionAlternativa = IConexiones.TipoConexion.Automatico
                                            .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                                    End Select

                                End If

                            Else

                                'De no existir una configuración, se pondrá por defecto SQL Server solamente en singleton

                                .OrigenDatosAlternativo = IConexiones.Controladores.SQLServer2008

                                .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject

                                .TipoConexionAlternativa = IConexiones.TipoConexion.Automatico

                                .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                            End If

                        End If

                    Catch ex As Exception

                        .OperadorCatalogoBorrado = Nothing

                        .OperadorCatalogoInsercion = Nothing

                        .OperadorCatalogoModificacion = Nothing

                        .IdentificadorEmpresa = "-1"

                    End Try

                    Return .Clone

                Else

                    tagWatcher_.SetError(TagWatcher.ErrorTypes.C3_001_3007)

                    Return Nothing

                End If

            End With

        End Function

        Public Function EnsamblaModulo(ByVal modulo_ As String) As IOperacionesCatalogo
            'bautismen
            Dim ioperacionesModulo_ As IOperacionesCatalogo = New OperacionesCatalogo

            Dim ejecuta_ As Boolean = False

            '' If System.IO.File.Exists(System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "\Configuracion\ConfiguracionGeneralModulos.xml") Then
            'If System.IO.File.Exists(System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml") Then

            '    ' _ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "\Configuracion\ConfiguracionGeneralModulos.xml"
            '_ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml"

            '    ejecuta_ = True

            '    ' Else

            '    '     If System.IO.File.Exists("C:\svn\SVN QA\Configuracion\ConfiguracionGeneralModulos.xml") Then
            '    '         'Lo buscamos en la ruta por defecto
            '    '         _ileerarchivo.RutaArchivo = "C:\svn\SVN QA\Configuracion\ConfiguracionGeneralModulos.xml"

            '    '         ejecuta_ = True

            '    '     End If

            ' End If

            ejecuta_ = True

            If ejecuta_ Then

                _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

                With ioperacionesModulo_

                    .TablaEdicion = _ileerarchivo.RegresaValor("TablaEdicion." & modulo_)

                    .IdentificadorCatalogo = _ileerarchivo.RegresaValor("CampoLlave." & modulo_)

                    .OperadorCatalogoConsulta = _ileerarchivo.RegresaValor("VT." & modulo_)

                    .VistaEncabezados = _ileerarchivo.RegresaValor("VE." & modulo_)

                    .Nombre = _ileerarchivo.RegresaValor("Descripcion." & modulo_)

                    .NombreToken = modulo_

                    .IdentificadorEmpresa = "-1"

                    Try

                        .IdentificadorEmpresa = _ileerarchivo.RegresaValor("Multiempresa." & modulo_)

                        .OperadorCatalogoBorrado = _ileerarchivo.RegresaValor("OperadorBorrado." & modulo_)

                        .OperadorCatalogoInsercion = _ileerarchivo.RegresaValor("OperadorInserciones." & modulo_)

                        .OperadorCatalogoModificacion = _ileerarchivo.RegresaValor("OperadorModificaciones." & modulo_)

                        .Version = _ileerarchivo.RegresaValor("Version." & modulo_)

                        .NombreClaveUpsert = _ileerarchivo.RegresaValor("NombreClaveUpsert." & modulo_)

                        .Granularidad = _ileerarchivo.RegresaValor("Granularidad." & modulo_)

                        .Entidad = _ileerarchivo.RegresaValor("Entidad." & modulo_)

                        .Dimension = _ileerarchivo.RegresaValor("Dimension." & modulo_)

                        'Valida la condiguración de la propiedad ReflejarEn, si es que viene como propiedad o del XML
                        If .ReflejarEn = IConexiones.Controladores.SinDefinir Then

                            If _ileerarchivo.Propiedades.ContainsKey("ReflejarEn." & modulo_) Then

                                Dim registroReflejarEn_ As String = _ileerarchivo.RegresaValor("ReflejarEn." & modulo_)

                                .ReflejarEn = DirectCast([Enum].Parse(GetType(IEnlaceDatos.DestinosParaReplicacion), registroReflejarEn_), IEnlaceDatos.DestinosParaReplicacion)

                            End If

                        End If

                        If _ileerarchivo.Propiedades.ContainsKey("OrigenDatos." & modulo_) Then

                            Dim valor_ As String = _ileerarchivo.RegresaValor("OrigenDatos." & modulo_)

                            If Not valor_ Is Nothing Then
                                valor_ = LCase(Trim(valor_))

                                Select Case valor_
                                    Case "mongodb", "mongo"

                                        With ioperacionesModulo_

                                            .OrigenDatos = IConexiones.Controladores.MongoDB
                                            .ObjetoDatos = IConexiones.TiposRepositorio.DataSetObject
                                            .TipoConexion = IConexiones.TipoConexion.DirectMongoDB
                                            .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.ConexionLibre

                                        End With

                                    Case "sqlserver", "mssql"

                                        With ioperacionesModulo_

                                            .OrigenDatos = IConexiones.Controladores.SQLServer2008
                                            .ObjetoDatos = IConexiones.TiposRepositorio.Automatico
                                            .TipoConexion = IConexiones.TipoConexion.Automatico
                                            .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                                        End With

                                End Select

                            End If

                        Else

                            'De no existir una configuración, se pondrá por defecto SQL Server solamente en singleton

                            .OrigenDatos = IConexiones.Controladores.SQLServer2008

                            .ObjetoDatos = IConexiones.TiposRepositorio.Automatico

                            .TipoConexion = IConexiones.TipoConexion.Automatico

                            .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                        End If

                        If _ileerarchivo.Propiedades.ContainsKey("OrigenDatosAlternativo." & modulo_) Then

                            Dim valor_ As String = _ileerarchivo.RegresaValor("OrigenDatosAlternativo." & modulo_)

                            If Not valor_ Is Nothing Then

                                valor_ = LCase(Trim(valor_))

                                Select Case valor_

                                    Case "mongodb", "mongo"

                                        With ioperacionesModulo_

                                            .OrigenDatosAlternativo = IConexiones.Controladores.MongoDB

                                            .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject

                                            .TipoConexionAlternativa = IConexiones.TipoConexion.DirectMongoDB

                                            .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.ConexionLibre

                                        End With

                                    Case "sqlserver", "mssql"

                                        With ioperacionesModulo_

                                            .OrigenDatosAlternativo = IConexiones.Controladores.SQLServer2008

                                            .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject

                                            .TipoConexionAlternativa = IConexiones.TipoConexion.Automatico

                                            .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                                        End With

                                End Select

                            End If

                        Else

                            'De no existir una configuración, se pondrá por defecto SQL Server solamente en singleton

                            .OrigenDatosAlternativo = IConexiones.Controladores.SQLServer2008

                            .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.Automatico

                            .TipoConexionAlternativa = IConexiones.TipoConexion.Automatico

                            .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                        End If

                    Catch ex As Exception

                        .OperadorCatalogoBorrado = Nothing

                        .OperadorCatalogoInsercion = Nothing

                        .OperadorCatalogoModificacion = Nothing

                        .IdentificadorEmpresa = "-1"

                    End Try

                    Return .Clone

                End With

            Else

                '                TagWatcher_.C3_001_3007

                Return Nothing

            End If

        End Function

        Public Function EnsamblaModulo(ByVal modulo_ As Organismo.Modulos) As IOperacionesCatalogo
            Dim ioperacionesModulo_ As IOperacionesCatalogo = New OperacionesCatalogo

            If Not _ileerarchivo.StatusArchivo = ILeerArchivo.StatusArchivos.Cargado Then

                '_ileerarchivo.RutaArchivo = ConfigurationManager.AppSettings("RutaConfiguracionGeneralModulos") & "ConfiguracionGeneralModulos.xml"
                ' _ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "\Configuracion\ConfiguracionGeneralModulos.xml"
                '_ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml"
                '
                '"C:\Wilma\esp\bin\Settings\ConfiguracionGeneralModulos.xml" 'Environment.GetEnvironmentVariable("appw2014") & "\Configuracion\ConfiguracionGeneralModulos.xml"

                '_ileerarchivo.LeerXML()
                _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

            End If

            ioperacionesModulo_.TablaEdicion = _ileerarchivo.RegresaValor("TablaEdicion." & modulo_.ToString)

            ioperacionesModulo_.IdentificadorCatalogo = _ileerarchivo.RegresaValor("CampoLlave." & modulo_.ToString)

            ioperacionesModulo_.OperadorCatalogoConsulta = _ileerarchivo.RegresaValor("VT." & modulo_.ToString)

            ioperacionesModulo_.VistaEncabezados = _ileerarchivo.RegresaValor("VE." & modulo_.ToString)

            ioperacionesModulo_.Nombre = _ileerarchivo.RegresaValor("Descripcion." & modulo_.ToString)

            ioperacionesModulo_.NombreToken = modulo_

            ioperacionesModulo_.IdentificadorEmpresa = "-1"

            'Valida la condiguración de la propiedad ReflejarEn, si es que viene como propiedad o del XML
            If ioperacionesModulo_.ReflejarEn = IConexiones.Controladores.SinDefinir Then

                If _ileerarchivo.Propiedades.ContainsKey("ReflejarEn." & modulo_.ToString) Then

                    Dim registroReflejarEn_ As String = _ileerarchivo.RegresaValor("ReflejarEn." & modulo_.ToString)

                    ioperacionesModulo_.ReflejarEn = DirectCast([Enum].Parse(GetType(IEnlaceDatos.DestinosParaReplicacion), registroReflejarEn_), IEnlaceDatos.DestinosParaReplicacion)

                End If

            End If

            Try

                ioperacionesModulo_.IdentificadorEmpresa = _ileerarchivo.RegresaValor("Multiempresa." & modulo_.ToString)

                ioperacionesModulo_.OperadorCatalogoBorrado = _ileerarchivo.RegresaValor("OperadorBorrado." & modulo_.ToString)

                ioperacionesModulo_.OperadorCatalogoInsercion = _ileerarchivo.RegresaValor("OperadorInserciones." & modulo_.ToString)

                ioperacionesModulo_.OperadorCatalogoModificacion = _ileerarchivo.RegresaValor("OperadorModificaciones." & modulo_.ToString)

            Catch ex As Exception

                ioperacionesModulo_.IdentificadorEmpresa = "-1"

                ioperacionesModulo_.OperadorCatalogoBorrado = Nothing

                ioperacionesModulo_.OperadorCatalogoInsercion = Nothing

                ioperacionesModulo_.OperadorCatalogoModificacion = Nothing

            End Try

            Return ioperacionesModulo_.Clone

        End Function

        Public Sub EnsamblaModulo(ByVal modulo_ As String,
                                  ByRef gsCatalogo_ As GsCatalogo)
            'Dim ioperacionesModulo_ As IOperacionesCatalogo = New OperacionesCatalogo

            If Not _ileerarchivo.StatusArchivo = ILeerArchivo.StatusArchivos.Cargado Then


                '_ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml"

                _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)


            End If

            'Generales
            Dim mensajeConfiguracion_ As String = "No se encuentra configurado el bloque [" & "VE." & modulo_ & "] en el archivo XML de configuración, verifique por favor"

            If _ileerarchivo.Propiedades.ContainsKey("VE." & modulo_) Then

                gsCatalogo_.VistaEncabezados = _ileerarchivo.RegresaValor("VE." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("VT." & modulo_) Then

                gsCatalogo_.OperadorCatalgosConsulta = _ileerarchivo.RegresaValor("VT." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("CampoLlave." & modulo_) Then

                gsCatalogo_.IdentificadorCatalogo = _ileerarchivo.RegresaValor("CampoLlave." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("TablaEdicion." & modulo_) Then

                gsCatalogo_.TablaEdicion = _ileerarchivo.RegresaValor("TablaEdicion." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("Descripcion." & modulo_) Then

                gsCatalogo_.NombreCatalogo = _ileerarchivo.RegresaValor("Descripcion." & modulo_)

                gsCatalogo_.OperacionesCatalogo.NombreToken = modulo_

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("Multiempresa." & modulo_) Then

                gsCatalogo_.IdentificadorEmpresa = _ileerarchivo.RegresaValor("Multiempresa." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If
            'Operadores

            If _ileerarchivo.Propiedades.ContainsKey("OperadorBorrado." & modulo_) Then

                gsCatalogo_.OperadorCatalogosBorrado = _ileerarchivo.RegresaValor("OperadorBorrado." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("OperadorInserciones." & modulo_) Then

                gsCatalogo_.OperadorCatalogosInsercion = _ileerarchivo.RegresaValor("OperadorInserciones." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If

            If _ileerarchivo.Propiedades.ContainsKey("OperadorModificaciones." & modulo_) Then

                gsCatalogo_.OperadorCatalogosModificaciones = _ileerarchivo.RegresaValor("OperadorModificaciones." & modulo_)

            Else : MsgBox(mensajeConfiguracion_) : End If


            'Permisos

            If _ileerarchivo.Propiedades.ContainsKey("PermisoActualizar." & modulo_) Then
                gsCatalogo_.TagActualizarConsulta = _ileerarchivo.RegresaValor("PermisoActualizar." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If



            If _ileerarchivo.Propiedades.ContainsKey("PermisoInsertar." & modulo_) Then
                gsCatalogo_.TagAgregar = _ileerarchivo.RegresaValor("PermisoInsertar." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If



            If _ileerarchivo.Propiedades.ContainsKey("PermisoBuscar." & modulo_) Then
                gsCatalogo_.TagBuscar = _ileerarchivo.RegresaValor("PermisoBuscar." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If



            If _ileerarchivo.Propiedades.ContainsKey("PermisoEliminar." & modulo_) Then
                gsCatalogo_.TagEliminar = _ileerarchivo.RegresaValor("PermisoEliminar." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If



            If _ileerarchivo.Propiedades.ContainsKey("PermisoModificar." & modulo_) Then
                gsCatalogo_.TagModificar = _ileerarchivo.RegresaValor("PermisoModificar." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If


            'Gestor

            If _ileerarchivo.Propiedades.ContainsKey("TipoEnsambladoGestor." & modulo_) Then
                gsCatalogo_.TipoEnsamblado = _ileerarchivo.RegresaValor("TipoEnsambladoGestor." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If


            If _ileerarchivo.Propiedades.ContainsKey("ArchivoGestor." & modulo_) Then
                gsCatalogo_.ArchivoEnsamblado = _ileerarchivo.RegresaValor("ArchivoGestor." & modulo_)
            Else : MsgBox(mensajeConfiguracion_) : End If


            If _ileerarchivo.Propiedades.ContainsKey("Version." & modulo_) Then
                gsCatalogo_.OperacionesCatalogo.Version = _ileerarchivo.RegresaValor("Version." & modulo_)
                'Else : MsgBox(mensajeConfiguracion_)
            End If

            If _ileerarchivo.Propiedades.ContainsKey("NombreClaveUpsert." & modulo_) Then
                gsCatalogo_.OperacionesCatalogo.NombreClaveUpsert = _ileerarchivo.RegresaValor("NombreClaveUpsert." & modulo_)
            End If

            If _ileerarchivo.Propiedades.ContainsKey("Granularidad." & modulo_) Then
                gsCatalogo_.OperacionesCatalogo.Granularidad = _ileerarchivo.RegresaValor("Granularidad." & modulo_)
                'Else : MsgBox(mensajeConfiguracion_)
            End If

            If _ileerarchivo.Propiedades.ContainsKey("Entidad." & modulo_) Then
                gsCatalogo_.OperacionesCatalogo.Entidad = _ileerarchivo.RegresaValor("Entidad." & modulo_)
                'Else : MsgBox(mensajeConfiguracion_)
            End If

            If _ileerarchivo.Propiedades.ContainsKey("Dimension." & modulo_) Then
                gsCatalogo_.OperacionesCatalogo.Dimension = _ileerarchivo.RegresaValor("Dimension." & modulo_)
                'Else : MsgBox(mensajeConfiguracion_)
            End If

            'Valida la condiguración de la propiedad ReflejarEn, si es que viene como propiedad o del XML
            If gsCatalogo_.OperacionesCatalogo.ReflejarEn = IConexiones.Controladores.SinDefinir Then

                If _ileerarchivo.Propiedades.ContainsKey("ReflejarEn." & modulo_) Then

                    Dim registroReflejarEn_ As String = _ileerarchivo.RegresaValor("ReflejarEn." & modulo_)

                    gsCatalogo_.OperacionesCatalogo.ReflejarEn = DirectCast([Enum].Parse(GetType(IEnlaceDatos.DestinosParaReplicacion), registroReflejarEn_), IEnlaceDatos.DestinosParaReplicacion)

                End If

            End If

            If _ileerarchivo.Propiedades.ContainsKey("OrigenDatos." & modulo_) Then
                Dim valor_ As String = _ileerarchivo.RegresaValor("OrigenDatos." & modulo_)

                If Not valor_ Is Nothing Then
                    valor_ = LCase(Trim(valor_))

                    Select Case valor_
                        Case "mongodb", "mongo"

                            With gsCatalogo_.OperacionesCatalogo

                                .OrigenDatos = IConexiones.Controladores.MongoDB
                                .ObjetoDatos = IConexiones.TiposRepositorio.DataSetObject
                                .TipoConexion = IConexiones.TipoConexion.DirectMongoDB
                                .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.ConexionLibre

                            End With



                        Case "sqlserver", "mssql"

                            With gsCatalogo_.OperacionesCatalogo

                                .OrigenDatos = IConexiones.Controladores.SQLServer2008
                                .ObjetoDatos = IConexiones.TiposRepositorio.Automatico
                                .TipoConexion = IConexiones.TipoConexion.Automatico
                                .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                            End With

                    End Select

                End If

            Else

                With gsCatalogo_.OperacionesCatalogo
                    'De no existir una configuración por defecto tomará SQL Server como origen 
                    .OrigenDatos = IConexiones.Controladores.SQLServer2008
                    .ObjetoDatos = IConexiones.TiposRepositorio.Automatico
                    .TipoConexion = IConexiones.TipoConexion.Automatico
                    .ModalidadConsulta = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir
                End With

            End If

            If _ileerarchivo.Propiedades.ContainsKey("OrigenDatosAlternativo." & modulo_) Then

                Dim valor_ As String = _ileerarchivo.RegresaValor("OrigenDatosAlternativo." & modulo_)

                If Not valor_ Is Nothing Then
                    valor_ = LCase(Trim(valor_))

                    Select Case valor_
                        Case "mongodb", "mongo"

                            With gsCatalogo_.OperacionesCatalogo

                                .OrigenDatosAlternativo = IConexiones.Controladores.MongoDB
                                .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject
                                .TipoConexionAlternativa = IConexiones.TipoConexion.DirectMongoDB
                                .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.ConexionLibre

                            End With



                        Case "sqlserver", "mssql"

                            With gsCatalogo_.OperacionesCatalogo

                                .OrigenDatosAlternativo = IConexiones.Controladores.SQLServer2008
                                .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.DataSetObject
                                .TipoConexionAlternativa = IConexiones.TipoConexion.Automatico
                                .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                            End With

                    End Select

                End If

            Else

                With gsCatalogo_.OperacionesCatalogo

                    'De no existir una configuración por defecto tomará SQL Server como origen 
                    .OrigenDatosAlternativo = IConexiones.Controladores.SQLServer2008
                    .ObjetoDatosAlternativo = IConexiones.TiposRepositorio.Automatico
                    .TipoConexionAlternativa = IConexiones.TipoConexion.Automatico
                    .ModalidadConsultaAlternativa = IOperacionesCatalogo.ModalidadesConsulta.SinDefinir

                End With

            End If

            If _ileerarchivo.Propiedades.ContainsKey("VentanasIndependientes." & modulo_) Then
                Dim valor_ As String = _ileerarchivo.RegresaValor("VentanasIndependientes." & modulo_)

                If Trim(LCase(valor_)) = "true" Then
                    gsCatalogo_.VentanasIndependientes = True
                ElseIf Trim(LCase(valor_)) = "false" Then
                    gsCatalogo_.VentanasIndependientes = False
                Else
                    gsCatalogo_.VentanasIndependientes = True
                End If

                'Else : MsgBox(mensajeConfiguracion_)
            End If

            If _ileerarchivo.Propiedades.ContainsKey("ActivarControlVentanasIndependientes." & modulo_) Then
                ' gsCatalogo_.ActivarControlVentanasIndependientes = _ileerarchivo.RegresaValor("ActivarControlVentanasIndependientes." & modulo_)

                Dim valor_ As String = _ileerarchivo.RegresaValor("ActivarControlVentanasIndependientes." & modulo_)

                If Trim(LCase(valor_)) = "true" Then
                    gsCatalogo_.ActivarControlVentanasIndependientes = True
                ElseIf Trim(LCase(valor_)) = "false" Then
                    gsCatalogo_.ActivarControlVentanasIndependientes = False
                Else
                    gsCatalogo_.ActivarControlVentanasIndependientes = False
                End If

            End If


            '            <Modulo ModalidadAmbiente.Seguimiento="1"/>
            '<Modulo ActivarControlAmbientes.Seguimiento="1"/>



            'Agrega los marcadores definidos del catálogo

            Dim marcadorAsignado_ As String = "MarcadorColumna1." & modulo_

            If _ileerarchivo.Propiedades.ContainsKey(marcadorAsignado_) Then

                AplicaMarcador(gsCatalogo_, marcadorAsignado_, _ileerarchivo.RegresaValor(marcadorAsignado_))

            End If

            marcadorAsignado_ = "MarcadorColumna2." & modulo_

            If _ileerarchivo.Propiedades.ContainsKey(marcadorAsignado_) Then

                AplicaMarcador(gsCatalogo_, marcadorAsignado_, _ileerarchivo.RegresaValor(marcadorAsignado_))

            End If

            marcadorAsignado_ = "MarcadorColumna3." & modulo_

            If _ileerarchivo.Propiedades.ContainsKey(marcadorAsignado_) Then

                AplicaMarcador(gsCatalogo_, marcadorAsignado_, _ileerarchivo.RegresaValor(marcadorAsignado_))

            End If

            marcadorAsignado_ = "MarcadorColumna4." & modulo_

            If _ileerarchivo.Propiedades.ContainsKey(marcadorAsignado_) Then

                AplicaMarcador(gsCatalogo_, marcadorAsignado_, _ileerarchivo.RegresaValor(marcadorAsignado_))

            End If

            marcadorAsignado_ = "MarcadorColumna5." & modulo_

            If _ileerarchivo.Propiedades.ContainsKey(marcadorAsignado_) Then

                AplicaMarcador(gsCatalogo_, marcadorAsignado_, _ileerarchivo.RegresaValor(marcadorAsignado_))

            End If

            'Prueba exitosa, pero no es conveniente mantener solo 30 registros cuando la latencia no mejora en apariencia
            ' gsCatalogo_.OperacionesCatalogo.CantidadVisibleRegistros = 30


        End Sub

        Public Sub AplicaMarcador(ByRef gsCatalogo_ As GsCatalogo,
                                  ByVal marcador_ As String,
                                  ByVal indicaciones_ As String)

            Dim marcador1_ As New gsol.Componentes.SistemaBase.GsCatalogo.MarcadorCelda

            'Dim indicacion_ As String = _ileerarchivo.RegresaValor(marcador_)

            If Not indicaciones_ Is Nothing Then

                Dim valores() As String = indicaciones_.Split("|")

                If valores.Length = 4 Then

                    marcador1_.NombreColumna = valores(0) '"Tipo de póliza"

                    marcador1_.CuandoValorIgualA = valores(1) '"Ingresos"

                    marcador1_.AplicarColorFondoCeldas = Color.FromName(valores(2)) 'Color.LightCyan

                    marcador1_.AplicarColorFondoTexto = Color.FromName(valores(3)) 'Color.Gray

                    gsCatalogo_.ListaMarcadoresColumnas.Add(marcador1_)

                Else

                    MsgBox("El marcador (" & marcador_ & ") debe contener una cadena de indicaciones con 4 parámetros,[Nombre columna, Valor, ColorCelda, ColorTexto]")

                End If

            Else

                MsgBox("El marcador (" & marcador_ & ") debe contener cadena de indicaciones asignada de lo contrario, por favor excluyalo")

            End If

        End Sub

        Public Function CargaModuloVirtual(ByVal tipo_ As String,
                                ByVal ioperaciones_ As IOperacionesCatalogo,
                                ByVal tipooperacion_ As IOperacionesCatalogo.TiposOperacionSQL) As IOperacionesCatalogo

            Dim nombretipo_ As String = Nothing
            Dim nombreensamblado_ As String = Nothing
            Dim indiceunico_ As String = "-2"

            Dim ioperacionescatalogo_ As IOperacionesCatalogo
            ioperacionescatalogo_ = New OperacionesCatalogo


            If _ileerarchivo.StatusArchivo = ILeerArchivo.StatusArchivos.Cargado Then

                nombretipo_ = _ileerarchivo.RegresaValor("Modulo." & tipo_)

                nombreensamblado_ = _ileerarchivo.RegresaValor("Ensamblado." & tipo_)

            Else

                '_ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml"

                _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

                nombretipo_ = _ileerarchivo.RegresaValor("Modulo." & tipo_)

                nombreensamblado_ = _ileerarchivo.RegresaValor("Ensamblado." & tipo_)

            End If

            'Le daremos preferencia a la ruta del bitecode, antes de consultar la configuración
            Dim _rutaConfigurada As String = nombreensamblado_

            ' nombreensamblado_ = LCase(nombreensamblado_).Replace("c:\svn\svn qa", System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString())
            nombreensamblado_ = LCase(nombreensamblado_).Replace("c:\svn\svn qa", System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString()).Replace("modulos\", "modules\")

            If Not System.IO.File.Exists(nombreensamblado_) Then

                nombreensamblado_ = _rutaConfigurada

                If Not System.IO.File.Exists(nombreensamblado_) Then

                    GsDialogo("Archivo no encontrado en contrado en la ruta indicada [" & nombreensamblado_ & "], por  favor reporte al administrador", Componentes.SistemaBase.GsDialogo.TipoDialogo.Alerta)

                    Return Nothing

                End If

            End If


            If Not (nombretipo_ Is Nothing And
                nombreensamblado_ Is Nothing) Then

                Dim objetocontenedor_ As [Assembly] = [Assembly].LoadFrom(nombreensamblado_)
                Dim tipocarga_ As Type = objetocontenedor_.GetType(nombretipo_)
                Dim instanciagenerica_ As Object

                'Dim argumentos_() As Object = {CType(ioperaciones_.EspacioTrabajo, Object)}
                Dim argumentos_() As Object = {CType(ioperaciones_, Object),
                                 CType(tipooperacion_, Object)}

                instanciagenerica_ = Activator.CreateInstance(tipocarga_, argumentos_)

                Dim formulario_ = instanciagenerica_

                instanciagenerica_.OperacionesCatalogo.ClausulasLibres = ioperaciones_.ClausulasLibres

                formulario_.ShowDialog()

                Try

                    ioperacionescatalogo_ = formulario_.OperacionesCatalogo

                Catch ex As Exception
                    'Log(ex.ToString, )
                End Try

            End If

            Return ioperacionescatalogo_

        End Function

        Public Function CargaModuloVirtual(ByVal tipo_ As String,
                                ByVal ioperaciones_ As IOperacionesCatalogo) As IOperacionesCatalogo

            Dim nombretipo_ As String = Nothing

            Dim nombreensamblado_ As String = Nothing

            Dim indiceunico_ As String = "-2"

            Dim usarGenerico_ As Boolean = False

            Dim ioperacionescatalogo_ As IOperacionesCatalogo

            ioperacionescatalogo_ = New OperacionesCatalogo

            If _ileerarchivo.StatusArchivo = ILeerArchivo.StatusArchivos.Cargado Then

                nombretipo_ = _ileerarchivo.RegresaValor("Modulo." & tipo_)

                nombreensamblado_ = _ileerarchivo.RegresaValor("Ensamblado." & tipo_)

            Else
                ' _ileerarchivo.RutaArchivo = System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString() & "..\settings\ConfiguracionGeneralModulos.xml"

                _ileerarchivo.LeerXML(ILeerArchivo.TiposAutomaticos.ConfiguracionModulos)

                nombretipo_ = _ileerarchivo.RegresaValor("Modulo." & tipo_)

                nombreensamblado_ = _ileerarchivo.RegresaValor("Ensamblado." & tipo_)

            End If

            If Not (nombretipo_ Is Nothing And
                nombreensamblado_ Is Nothing) Then

                'Le daremos preferencia a la ruta del bitecode, antes de consultar la configuración
                Dim _rutaConfigurada As String = nombreensamblado_

                ' nombreensamblado_ = LCase(nombreensamblado_).Replace("c:\svn\svn qa", System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString())
                nombreensamblado_ = LCase(nombreensamblado_).Replace("c:\svn\svn qa", System.IO.Directory.GetParent(Environment.CurrentDirectory).ToString()).Replace("modulos\", "modules\")

                If Not System.IO.File.Exists(nombreensamblado_) Then

                    nombreensamblado_ = _rutaConfigurada

                    If Not System.IO.File.Exists(nombreensamblado_) Then

                        GsDialogo("Archivo no encontrado en contrado en la ruta indicada [" & nombreensamblado_ & "], por  favor reporte al administrador", Componentes.SistemaBase.GsDialogo.TipoDialogo.Alerta)

                        Return Nothing

                    End If

                End If

                Dim objetocontenedor_ As [Assembly] = [Assembly].LoadFrom(nombreensamblado_)

                Dim tipocarga_ As Type = objetocontenedor_.GetType(nombretipo_)
                Dim instanciagenerica_ As Object

                If nombretipo_ = "Gsol.Modulos.GenericCatalog64.Frm000Generic" Or
                    nombretipo_ = "Wma.Modules.GenericCatalog64.Frm000Generic" Then

                    usarGenerico_ = True

                End If

                Dim argumentos_() As Object

                If usarGenerico_ Then
                    argumentos_ = {CType(ioperaciones_.EspacioTrabajo, Object),
                                   CType(tipo_, String)}
                Else
                    argumentos_ = {CType(ioperaciones_.EspacioTrabajo, Object)}
                End If


                instanciagenerica_ = Activator.CreateInstance(tipocarga_, argumentos_)

                Dim formulario_ = instanciagenerica_

                'Dim frm As Form = CType(GenericInstance, Form)

                instanciagenerica_.OperacionesCatalogo.ClausulasLibres = ioperaciones_.ClausulasLibres

                'MsgBox("?" & ioperaciones_.ClausulasLibres)
                formulario_.ShowDialog()

                Try

                    'MsgBox("Sistema terminó " & forumulario_.IndiceUnico)

                    ioperacionescatalogo_ = formulario_.OperacionesCatalogo


                    'indiceunico_ = forumulario_.IndiceUnico
                Catch ex As Exception
                    'Log(ex.ToString, )
                End Try

            End If

            Return ioperacionescatalogo_

        End Function

        Public Function GsDialogo(ByVal mensaje_ As String,
                Optional ByVal tipodialogo_ As GsDialogo.TipoDialogo = Componentes.SistemaBase.GsDialogo.TipoDialogo.Aviso) As GsDialogo.Contestacion
            Dim contestacion_ As GsDialogo.Contestacion = Componentes.SistemaBase.GsDialogo.Contestacion.Cancelar

            'Instanciando Ventana de dialogo
            _ventanadialogo = New GsDialogo

            Return _ventanadialogo.Invocar(mensaje_,
                  tipodialogo_)

        End Function

        Public Function GsDialogo(ByRef respuesta_ As String,
                                  ByVal mensaje_ As String,
        Optional ByVal tipodialogo_ As GsDialogo.TipoDialogo = Componentes.SistemaBase.GsDialogo.TipoDialogo.Aviso) As GsDialogo.Contestacion
            Dim contestacion_ As GsDialogo.Contestacion = Componentes.SistemaBase.GsDialogo.Contestacion.Cancelar

            'Instanciando Ventana de dialogo
            _ventanadialogo = New GsDialogo

            Return _ventanadialogo.Invocar(respuesta_, mensaje_,
                  tipodialogo_)

        End Function

        Public Function GsDialogo(ByRef respuesta_ As String,
                                  ByRef richTextBox_ As RichTextBox,
                                  Optional ByVal tipodialogo_ As GsDialogo.TipoDialogo = Componentes.SistemaBase.GsDialogo.TipoDialogo.Aviso) As GsDialogo.Contestacion
            Dim contestacion_ As GsDialogo.Contestacion = Componentes.SistemaBase.GsDialogo.Contestacion.Cancelar

            'Instanciando Ventana de dialogo
            _ventanadialogo = New GsDialogo

            Return _ventanadialogo.Invocar(respuesta_, richTextBox_,
                  tipodialogo_)

        End Function

        Public Sub Log(
                      ByVal mensaje_ As String,
                       ByVal tipoBitacora_ As IBitacoras.TiposBitacora,
                       ByVal tipoSuceso_ As IBitacoras.TiposSucesos,
                       Optional ByVal credenciales_ As ICredenciales = Nothing,
                       Optional ByVal permiso_ As Int32 = Nothing,
                       Optional ByVal parametros_ As String = Nothing,
                       Optional ByVal disparo_ As String = Nothing,
                       Optional ByVal modulo_ As String = Nothing,
                       Optional ByVal nombreUsuario_ As String = Nothing
                        )

            _bitacora = New BitacoraCapaDatos(disparo_, credenciales_, permiso_, mensaje_, parametros_, tipoBitacora_, tipoSuceso_)

            _bitacora.Modulo = modulo_

            _bitacora.NombreUsuario = nombreUsuario_

            If _bitacora.Estatus = IBitacoras.EstatusInsercion.Incorrecto Then

                _bitacora = New BitacoraCapaDatos(mensaje_, tipoBitacora_, tipoSuceso_, nombreUsuario_, modulo_)

            End If

        End Sub

        'Genera una contraseña aleatoria
        Public Function GegeraContrasena(ByVal NivelContrasena As NivelContrasena) As String

            Dim contrasena_ As String = ""

            Select Case NivelContrasena

                'Crea una contraseña con 2 minusculas, 2 mayúscula y tener 8 caracteres
                Case Organismo.NivelContrasena.baja

                    Dim tipoCaracteres_ As New Dictionary(Of String, Int16) From {{"minusculas", 0}, {"mayusculas", 0}}

                    contrasena_ = GeneraCaracteresAleatorios(tipoCaracteres_, 6)

                    'Crea una contraseña con 2 minusculas, 2 mayúscula, 2 números y tener 8 caracteres
                Case Organismo.NivelContrasena.media

                    Dim tipoCaracteres_ As New Dictionary(Of String, Int16) From {{"minusculas", 0}, {"mayusculas", 0}, {"numeros", 0}}

                    contrasena_ = GeneraCaracteresAleatorios(tipoCaracteres_, 8)

                    'Crea una contraseña con 2 minusculas, 2 mayúscula, 2 números, 2 caracteres especiales y tener 10 caracteres
                Case Organismo.NivelContrasena.alta

                    Dim tipoCaracteres_ As New Dictionary(Of String, Int16) From {{"minusculas", 0}, {"mayusculas", 0}, {"numeros", 0}, {"especiales", 0}}

                    contrasena_ = GeneraCaracteresAleatorios(tipoCaracteres_, 10)

            End Select

            Return contrasena_

        End Function

        'Metodo que ocupa la función GegeraContrasena para obtener la contraseña aleatoria
        Private Function GeneraCaracteresAleatorios(ByVal tipoCaracteres_ As Dictionary(Of String, Int16), ByVal tamanoCadena_ As Int16)

            Dim aleatorio_ As New Random

            Dim contrasena_ As String = ""

            Dim caracter_ As Char

            While contrasena_.Length < tamanoCadena_

                Dim tipoCaracter_ As String = tipoCaracteres_.Keys(aleatorio_.Next(tipoCaracteres_.Count))

                If tipoCaracteres_.Count > 0 Then

                    If tipoCaracteres_.Item(tipoCaracter_) <= 2 Then

                        Select Case tipoCaracter_

                            Case "minusculas"
                                caracter_ = Chr(aleatorio_.Next(97, 122))

                            Case "mayusculas"
                                caracter_ = Chr(aleatorio_.Next(65, 90))

                            Case "numeros"
                                caracter_ = Chr(aleatorio_.Next(48, 57))

                            Case "especiales"
                                Dim caracteresEspeciales_ As New List(Of Int32) From {33, 35, 36, 37, 38, 42, 45, 46, 64, 95}
                                caracter_ = Chr(caracteresEspeciales_(aleatorio_.Next(caracteresEspeciales_.Count)))

                            Case Else
                                caracter_ = Chr(aleatorio_.Next(97, 122))

                        End Select

                        contrasena_ &= caracter_.ToString()

                        tipoCaracteres_.Item(tipoCaracter_) += 1

                    End If

                    If tipoCaracteres_.Item(tipoCaracter_) = 2 Then

                        tipoCaracteres_.Remove(tipoCaracter_)

                    End If

                Else

                    caracter_ = Chr(aleatorio_.Next(97, 122))

                    contrasena_ &= caracter_.ToString()

                End If

            End While

            Return contrasena_

        End Function

        Public Function ObtenerReglasCarteraClientes(ByVal ClaveUsuario_ As Integer,
                                             ByVal ClaveDivisionMiempresa_ As Integer,
                                             ByVal ioperaciones_ As OperacionesCatalogo,
                                             Optional VariableRFC_ As String = "sdie_t_RFC",
                                             Optional VariableClaveCliente_ As String = "i_Cve_ClienteOperativoExterno",
                                             Optional VariableTipoOperacion_ As String = "sp_i_TipoOperacion") As String

            Dim sistema_ = New Organismo

            Dim reglasCarteraSQL_ As String = Nothing

            Dim query_ = IIf(ClaveDivisionMiempresa_ > 0, " and i_Cve_DivisionMiEmpresa = " & ClaveDivisionMiempresa_, "") &
                        " and i_Cve_Usuario =  " & ClaveUsuario_

            Dim IOReglasCarteraClientes = New OperacionesCatalogo

            IOReglasCarteraClientes.EspacioTrabajo = ioperaciones_.EspacioTrabajo

            IOReglasCarteraClientes.CantidadVisibleRegistros = 0

            IOReglasCarteraClientes = sistema_.ConsultaModulo(IOReglasCarteraClientes.EspacioTrabajo,
                                                              "ResumenReglasCuentaCliente",
                                                              query_)

            If sistema_.TieneResultados(IOReglasCarteraClientes) Then

                reglasCarteraSQL_ = " and ( "

                Dim contador_ As Integer = 0

                For Each fila_ As DataRow In IOReglasCarteraClientes.Vista.Tables(0).Rows

                    If contador_ > 0 Then

                        reglasCarteraSQL_ = reglasCarteraSQL_ & " or "

                    End If

                    If fila_.Item("Clave TipoOperacion").ToString = 1 Then

                        reglasCarteraSQL_ = reglasCarteraSQL_ & " ( " & VariableTipoOperacion_ & " = 1 and "

                    ElseIf fila_.Item("Clave TipoOperacion").ToString = 2 Then

                        reglasCarteraSQL_ = reglasCarteraSQL_ & " ( " & VariableTipoOperacion_ & " = 2 and "

                    Else

                        reglasCarteraSQL_ = reglasCarteraSQL_ & " (  "

                    End If

                    reglasCarteraSQL_ = reglasCarteraSQL_ & " " & VariableRFC_ & " = '" & fila_.Item("RFC").ToString & "'"

                    If IsNumeric(fila_.Item("Clave ClienteExterno").ToString) Then

                        reglasCarteraSQL_ = reglasCarteraSQL_ & " and " & VariableClaveCliente_ & " = " & fila_.Item("Clave ClienteExterno").ToString

                    End If

                    reglasCarteraSQL_ = reglasCarteraSQL_ & IIf(ClaveDivisionMiempresa_ > 0, " and i_Cve_DivisionMiEmpresa = " & ClaveDivisionMiempresa_ & ")", ")")

                    contador_ = contador_ + 1

                Next

                reglasCarteraSQL_ = reglasCarteraSQL_ & " ) "

            Else

                reglasCarteraSQL_ = Nothing

            End If

            Return reglasCarteraSQL_

        End Function

        Public Sub AbrirVisorDocumentos(ByRef gscCatalogo As GsCatalogo)

            Dim sistema_ = New Organismo

            gscCatalogo.ActivarGestor = False

            gscCatalogo.OperacionesCatalogo.AdvertenciasIndicador = False

            If gscCatalogo.OperacionesCatalogo.CampoPorNombre("i_Cve_Documento") <> Nothing Then

                gscCatalogo.OperacionesCatalogo.AdvertenciasIndicador = True

                Dim operacionesDocumento_ = New OperacionesDocumento64(gscCatalogo.OperacionesCatalogo.Clone)

                operacionesDocumento_.Documento.Clave = gscCatalogo.OperacionesCatalogo.CampoPorNombre("i_Cve_Documento")

                operacionesDocumento_.VisualizarDocumento()

                If operacionesDocumento_.Estatus.Status = TagWatcher.TypeStatus.Errors Then

                    sistema_.GsDialogo(operacionesDocumento_.Estatus.ErrorDescription, Componentes.SistemaBase.GsDialogo.TipoDialogo.Err)

                End If

            Else

                sistema_.GsDialogo("¡Seleccione el documento a visualizar!", Componentes.SistemaBase.GsDialogo.TipoDialogo.Aviso)

                gscCatalogo.OperacionesCatalogo.AdvertenciasIndicador = True

            End If

        End Sub

        Public Sub AbrirVisorDocumentos(ByVal ioperaciones_ As OperacionesCatalogo,
                         ByVal claveDocumento_ As Integer)

            Dim sistema_ = New Organismo

            Dim operacionesDocumento_ = New OperacionesDocumento64(ioperaciones_.Clone)

            operacionesDocumento_.Documento.Clave = claveDocumento_

            operacionesDocumento_.VisualizarDocumento()

            If operacionesDocumento_.Estatus.Status = TagWatcher.TypeStatus.Errors Then

                sistema_.GsDialogo(operacionesDocumento_.Estatus.ErrorDescription, Componentes.SistemaBase.GsDialogo.TipoDialogo.Err)

            End If

        End Sub

        Public Function ObtenerGiroEmpresarial(ByVal ioperaciones_ As OperacionesCatalogo) As Integer

            Dim ioperacionesMisEmpresas_ As New OperacionesCatalogo

            Dim cveGiroEmpresarial_ As Integer = Nothing

            Dim sistema_ = New Organismo

            ioperacionesMisEmpresas_.EspacioTrabajo = ioperaciones_.EspacioTrabajo

            ioperacionesMisEmpresas_.CantidadVisibleRegistros = 0

            ioperacionesMisEmpresas_ = sistema_.ConsultaModulo(ioperacionesMisEmpresas_.EspacioTrabajo,
                                                           "MisEmpresas",
                                                           " AND i_Cve_DivisionMiEmpresa = " & ioperacionesMisEmpresas_.EspacioTrabajo.MisCredenciales.DivisionEmpresaria)

            If TieneResultados(ioperacionesMisEmpresas_) Then

                cveGiroEmpresarial_ = ioperacionesMisEmpresas_.Vista(0, "i_Cve_GiroEmpresarial", IOperacionesCatalogo.TiposAccesoCampo.NombreTecnico)

            End If

            Return cveGiroEmpresarial_

        End Function

        Public Sub GeneraLog(ByVal rutaArchivo As String, ByVal texto As String)

            Dim ruta_ As String = rutaArchivo & ".log"

            Using fileStream_ As FileStream = File.Open(ruta_, FileMode.Append)

                Dim texto_ As Byte() = New UTF8Encoding(True).GetBytes(texto)

                fileStream_.Write(texto_, 0, texto_.Length)

            End Using

        End Sub

#End Region

    End Class



    '    Imports System
    'Imports System.Text
    'Imports System.Globalization

    ''' <summary>
    ''' Convierte números en su expresión numérica a su numeral cardinal
    ''' </summary>
    Public NotInheritable Class NumeroLetras

#Region "Miembros estáticos"

        Private Const UNI As Integer = 0, DIECI As Integer = 1, DECENA As Integer = 2, CENTENA As Integer = 3
        Private Shared _matriz As String(,) = New String(CENTENA, 9) {
            {Nothing, " uno", " dos", " tres", " cuatro", " cinco", " seis", " siete", " ocho", " nueve"},
            {" diez", " once", " doce", " trece", " catorce", " quince", " dieciséis", " diecisiete", " dieciocho", " diecinueve"},
            {Nothing, Nothing, Nothing, " treinta", " cuarenta", " cincuenta", " sesenta", " setenta", " ochenta", " noventa"},
            {Nothing, Nothing, Nothing, Nothing, Nothing, " quinientos", Nothing, " setecientos", Nothing, " novecientos"}}

        Private Shared _matriz2 As String(,) = New String(CENTENA, 9) {
            {Nothing, " one", " two", " three", " four", " five", " six", " seven", " eight", " nine"},
            {" ten", " eleven", " twelve", " thirteen", " fourteen", " fifteen", " sixteen", " seventeen", " seventeen", " nineteen"},
            {Nothing, Nothing, Nothing, " thirty ", " fourty", " fifty", " sixty", " seventy", " eighty", " ninety"},
            {Nothing, Nothing, Nothing, Nothing, Nothing, " five hundred", Nothing, " seven hundred", Nothing, " nine hundred"}} ' Andrea para importe en ingles

        Private Const [sub] As Char = CChar(ChrW(26))
        'Cambiar acá si se quiere otro comportamiento en los métodos de clase
        Public Const SeparadorDecimalSalidaDefault As String = "con"
        Public Const MascaraSalidaDecimalDefault As String = "00'/100.-'"
        Public Const DecimalesDefault As Int32 = 2
        Public Const LetraCapitalDefault As Boolean = False
        Public Const ConvertirDecimalesDefault As Boolean = False
        Public Const ApocoparUnoParteEnteraDefault As Boolean = False
        Public Const ApocoparUnoParteDecimalDefault As Boolean = False

#End Region

#Region "Propiedades"

        Private _decimales As Int32 = DecimalesDefault
        Private _cultureInfo As CultureInfo = Globalization.CultureInfo.CurrentCulture
        Private _separadorDecimalSalida As String = SeparadorDecimalSalidaDefault
        Private _posiciones As Int32 = DecimalesDefault
        Private _mascaraSalidaDecimal As String, _mascaraSalidaDecimalInterna As String = MascaraSalidaDecimalDefault
        Private _esMascaraNumerica As Boolean = True
        Private _letraCapital As Boolean = LetraCapitalDefault
        Private _convertirDecimales As Boolean = ConvertirDecimalesDefault
        Private _apocoparUnoParteEntera As Boolean = False
        Private _apocoparUnoParteDecimal As Boolean

        ''' <summary>
        ''' Indica la cantidad de decimales que se pasarán a entero para la conversión
        ''' </summary>
        ''' <remarks>Esta propiedad cambia al cambiar MascaraDecimal por un valor que empieze con '0'</remarks>
        Public Property Decimales() As Int32
            Get
                Return _decimales
            End Get
            Set(ByVal value As Int32)
                If value > 10 Then
                    Throw New ArgumentException(value.ToString() + " excede el número máximo de decimales admitidos, solo se admiten hasta 10.")
                End If
                _decimales = value
            End Set
        End Property

        ''' <summary>
        ''' Objeto CultureInfo utilizado para convertir las cadenas de entrada en números
        ''' </summary>
        Public Property CultureInfo() As CultureInfo
            Get
                Return _cultureInfo
            End Get
            Set(ByVal value As CultureInfo)
                _cultureInfo = value
            End Set
        End Property

        ''' <summary>
        ''' Indica la cadena a intercalar entre la parte entera y la decimal del número
        ''' </summary>
        Public Property SeparadorDecimalSalida() As String
            Get
                Return _separadorDecimalSalida
            End Get
            Set(ByVal value As String)
                _separadorDecimalSalida = value
                'Si el separador decimal es compuesto, infiero que estoy cuantificando algo,
                'por lo que apocopo el "uno" convirtiéndolo en "un"
                If value.Trim().IndexOf(" ") > 0 Then
                    _apocoparUnoParteEntera = True
                Else
                    _apocoparUnoParteEntera = False
                End If
            End Set
        End Property

        ''' <summary>
        ''' Indica el formato que se le dara a la parte decimal del número
        ''' </summary>
        Public Property MascaraSalidaDecimal() As String
            Get
                If Not [String].IsNullOrEmpty(_mascaraSalidaDecimal) Then
                    Return _mascaraSalidaDecimal
                Else
                    Return ""
                End If
            End Get
            Set(ByVal value As String)
                'determino la cantidad de cifras a redondear a partir de la cantidad de '0' o ''
                'que haya al principio de la cadena, y también si es una máscara numérica
                Dim i As Integer = 0
                While i < value.Length AndAlso (value(i) = "0"c OrElse value(i) = "#")
                    i += 1
                End While
                _posiciones = i
                If i > 0 Then
                    _decimales = i
                    _esMascaraNumerica = True
                Else
                    _esMascaraNumerica = False
                End If
                _mascaraSalidaDecimal = value
                If _esMascaraNumerica Then
                    _mascaraSalidaDecimalInterna = value.Substring(0, _posiciones) + "'" + value.Substring(_posiciones).Replace("''", [sub].ToString()).Replace("'", [String].Empty).Replace([sub].ToString(), "'") + "'"
                Else
                    _mascaraSalidaDecimalInterna = value.Replace("''", [sub].ToString()).Replace("'", [String].Empty).Replace([sub].ToString(), "'")
                End If
            End Set
        End Property

        ''' <summary>
        ''' Indica si la primera letra del resultado debe estár en mayúscula
        ''' </summary>
        Public Property LetraCapital() As Boolean
            Get
                Return _letraCapital
            End Get
            Set(ByVal value As Boolean)
                _letraCapital = value
            End Set
        End Property

        ''' <summary>
        ''' Indica si se deben convertir los decimales a su expresión nominal
        ''' </summary>
        Public Property ConvertirDecimales() As Boolean
            Get
                Return _convertirDecimales
            End Get
            Set(ByVal value As Boolean)
                _convertirDecimales = value
                _apocoparUnoParteDecimal = value
                If value Then
                    ' Si la máscara es la default, la borro
                    If _mascaraSalidaDecimal = MascaraSalidaDecimalDefault Then
                        MascaraSalidaDecimal = ""
                    End If
                ElseIf [String].IsNullOrEmpty(_mascaraSalidaDecimal) Then
                    MascaraSalidaDecimal = MascaraSalidaDecimalDefault
                    'Si no hay máscara dejo la default
                End If
            End Set
        End Property

        ''' <summary>
        ''' Indica si de debe cambiar "uno" por "un" en las unidades.
        ''' </summary>
        Public Property ApocoparUnoParteEntera() As Boolean
            Get
                Return _apocoparUnoParteEntera
            End Get
            Set(ByVal value As Boolean)
                _apocoparUnoParteEntera = value
            End Set
        End Property

        ''' <summary>
        ''' Determina si se debe apococopar el "uno" en la parte decimal
        ''' </summary>
        ''' <remarks>El valor de esta propiedad cambia al setear ConvertirDecimales</remarks>
        Public Property ApocoparUnoParteDecimal() As Boolean
            Get
                Return _apocoparUnoParteDecimal
            End Get
            Set(ByVal value As Boolean)
                _apocoparUnoParteDecimal = value
            End Set
        End Property

#End Region

#Region "Constructores"

        Public Sub New()
            MascaraSalidaDecimal = MascaraSalidaDecimalDefault
            SeparadorDecimalSalida = SeparadorDecimalSalidaDefault
            LetraCapital = LetraCapitalDefault
            ConvertirDecimales = _convertirDecimales
        End Sub

        Public Sub New(ByVal ConvertirDecimales As Boolean, ByVal MascaraSalidaDecimal As String, ByVal SeparadorDecimalSalida As String, ByVal LetraCapital As Boolean)
            If Not [String].IsNullOrEmpty(MascaraSalidaDecimal) Then
                Me.MascaraSalidaDecimal = MascaraSalidaDecimal
            End If
            If Not [String].IsNullOrEmpty(SeparadorDecimalSalida) Then
                _separadorDecimalSalida = SeparadorDecimalSalida
            End If
            _letraCapital = LetraCapital
            _convertirDecimales = ConvertirDecimales
        End Sub

#End Region

#Region "Conversores de instancia"
        ' Andrea para importe en ingles se agrega el parametro opcional idioma 
        Public Function ToCustomCardinal(ByVal Numero As Double, Optional ByVal Idioma As Boolean = False) As String
            Return Convertir(Convert.ToDecimal(Numero), _decimales, _separadorDecimalSalida, _mascaraSalidaDecimalInterna, _esMascaraNumerica, _letraCapital,
            _convertirDecimales, _apocoparUnoParteEntera, _apocoparUnoParteDecimal, Idioma)
        End Function


        Public Function ToCustomCardinal(ByVal Numero As Double) As String
            Return Convertir(Convert.ToDecimal(Numero), _decimales, _separadorDecimalSalida, _mascaraSalidaDecimalInterna, _esMascaraNumerica, _letraCapital,
            _convertirDecimales, _apocoparUnoParteEntera, _apocoparUnoParteDecimal)
        End Function

        Public Function ToCustomCardinal(ByVal Numero As String, Optional ByVal Idioma As Boolean = False) As String
            Dim dNumero As Double
            If [Double].TryParse(Numero, NumberStyles.Float, _cultureInfo, dNumero) Then
                Return ToCustomCardinal(dNumero, Idioma)
            Else
                Throw New ArgumentException("'" + Numero + "' no es un número válido.")
            End If
        End Function

        Public Function ToCustomCardinal(ByVal Numero As Decimal, Optional ByVal Idioma As Boolean = False) As String
            Return ToCardinal(Numero, Idioma)
        End Function

        Public Function ToCustomCardinal(ByVal Numero As Int32, Optional ByVal Idioma As Boolean = False) As String
            Return Convertir(Convert.ToDecimal(Numero), 0, _separadorDecimalSalida, _mascaraSalidaDecimalInterna, _esMascaraNumerica, _letraCapital,
            _convertirDecimales, _apocoparUnoParteEntera, False, Idioma)
        End Function

#End Region

#Region "Conversores estáticos"
        ' Andrea para importe en ingles se agrega el parametro opcional idioma 
        Public Shared Function ToCardinal(ByVal Numero As Int32, Optional ByVal Idioma As Boolean = False) As String
            Return Convertir(Convert.ToDecimal(Numero), 0, Nothing, Nothing, True, LetraCapitalDefault,
            ConvertirDecimalesDefault, ApocoparUnoParteEnteraDefault, ApocoparUnoParteDecimalDefault, Idioma)
        End Function

        Public Shared Function ToCardinal(ByVal Numero As Double, Optional ByVal Idioma As Boolean = False) As String
            Return Convertir(Convert.ToDecimal(Numero), DecimalesDefault, SeparadorDecimalSalidaDefault, MascaraSalidaDecimalDefault, True, LetraCapitalDefault,
            ConvertirDecimalesDefault, ApocoparUnoParteEnteraDefault, ApocoparUnoParteDecimalDefault, Idioma)
        End Function

        Public Shared Function ToCardinal(ByVal Numero As String, ByVal ReferenciaCultural As CultureInfo, Optional ByVal Idioma As Boolean = False) As String
            Dim dNumero As Double
            If [Double].TryParse(Numero, NumberStyles.Float, ReferenciaCultural, dNumero) Then
                Return ToCardinal(dNumero, Idioma)
            Else
                Throw New ArgumentException("'" + Numero + "' no es un número válido.")
            End If
        End Function

        Public Shared Function ToCardinal(ByVal Numero As String, Optional ByVal Idioma As Boolean = False) As String
            Return NumeroLetras.ToCardinal(Numero, CultureInfo.CurrentCulture, Idioma)
        End Function

        Public Shared Function ToCardinal(ByVal Numero As Decimal, Optional ByVal Idioma As Boolean = False) As String
            Return ToCardinal(Convert.ToDouble(Numero), Idioma)
        End Function

#End Region

        Private Shared Function Convertir(ByVal Numero As Decimal, ByVal Decimales As Int32, ByVal SeparadorDecimalSalida As String, ByVal MascaraSalidaDecimal As String, ByVal EsMascaraNumerica As Boolean, ByVal LetraCapital As Boolean,
        ByVal ConvertirDecimales As Boolean, ByVal ApocoparUnoParteEntera As Boolean, ByVal ApocoparUnoParteDecimal As Boolean, Optional ByVal Idioma As Boolean = False) As String
            Dim Num As Int64
            Dim terna As Int32, centenaTerna As Int32, decenaTerna As Int32, unidadTerna As Int32, iTerna As Int32
            Dim cadTerna As String
            Dim Resultado As New StringBuilder()

            Num = Math.Floor(Math.Abs(Numero))

            If Num >= 1000000000001 OrElse Num < 0 Then
                Throw New ArgumentException("El número '" + Numero.ToString() + "' excedió los límites del conversor: [0;1.000.000.000.001]")
            End If
            If Num = 0 Then
                If Idioma = False Then
                    Resultado.Append(" cero")
                Else
                    Resultado.Append(" zero")
                End If

            Else
                iTerna = 0

                Do Until Num = 0

                    iTerna += 1
                    cadTerna = String.Empty
                    terna = Num Mod 1000

                    centenaTerna = Int(terna / 100)
                    decenaTerna = terna - centenaTerna * 100 'Decena junto con la unidad
                    unidadTerna = (decenaTerna - Math.Floor(decenaTerna / 10) * 10)

                    Select Case decenaTerna
                        Case 1 To 9
                            If Idioma = False Then
                                cadTerna = _matriz(UNI, unidadTerna) + cadTerna
                            Else
                                cadTerna = _matriz2(UNI, unidadTerna) + cadTerna
                            End If

                        Case 10 To 19
                            If Idioma = False Then
                                cadTerna = cadTerna + _matriz(DIECI, unidadTerna)
                            Else
                                cadTerna = cadTerna + _matriz2(DIECI, unidadTerna)
                            End If

                        Case 20
                            If Idioma = False Then
                                cadTerna = cadTerna + " veinte"
                            Else
                                cadTerna = cadTerna + " twenty"
                            End If

                        Case 21 To 29
                            If Idioma = False Then
                                cadTerna = " veinti" + _matriz(UNI, unidadTerna).Substring(1)
                            Else
                                cadTerna = " twenty" + _matriz2(UNI, unidadTerna).Substring(1)
                            End If

                        Case 30 To 99
                            If unidadTerna <> 0 Then
                                If Idioma = False Then
                                    cadTerna = _matriz(DECENA, Int(decenaTerna / 10)) + " y" + _matriz(UNI, unidadTerna) + cadTerna
                                Else
                                    cadTerna = _matriz2(DECENA, Int(decenaTerna / 10)) + "" + _matriz2(UNI, unidadTerna) + cadTerna
                                End If

                            Else
                                If Idioma = False Then
                                    cadTerna += _matriz(DECENA, Int(decenaTerna / 10))
                                Else
                                    cadTerna += _matriz2(DECENA, Int(decenaTerna / 10))
                                End If

                            End If
                    End Select

                    Select Case centenaTerna
                        Case 1
                            If Idioma = False Then
                                If decenaTerna > 0 Then
                                    cadTerna = " ciento" + cadTerna
                                Else
                                    cadTerna = " cien" + cadTerna
                                End If

                            Else
                                cadTerna = " one hundred" + cadTerna
                            End If
                            Exit Select
                        Case 5, 7, 9
                            If Idioma = False Then
                                cadTerna = _matriz(CENTENA, Int(terna / 100)) + cadTerna
                            Else
                                cadTerna = _matriz2(CENTENA, Int(terna / 100)) + cadTerna
                            End If

                            Exit Select
                        Case Else
                            If Int(terna / 100) > 1 Then
                                If Idioma = False Then
                                    cadTerna = _matriz(UNI, Int(terna / 100)) + "cientos" + cadTerna
                                Else
                                    cadTerna = _matriz2(UNI, Int(terna / 100)) + " hundred" + cadTerna
                                End If

                            End If
                            Exit Select
                    End Select
                    'Reemplazo el 'uno' por 'un' si no es en las únidades o si se solicító apocopar
                    If Idioma = False Then
                        If (iTerna > 1 OrElse ApocoparUnoParteEntera) AndAlso decenaTerna = 21 Then
                            cadTerna = cadTerna.Replace("veintiuno", "veintiún")
                        ElseIf (iTerna > 1 OrElse ApocoparUnoParteEntera) AndAlso unidadTerna = 1 AndAlso decenaTerna <> 11 Then
                            cadTerna = cadTerna.Substring(0, cadTerna.Length - 1)
                            'Acentúo 'veintidós', 'veintitrés' y 'veintiséis'
                        ElseIf decenaTerna = 22 Then
                            cadTerna = cadTerna.Replace("veintidos", "veintidós")
                        ElseIf decenaTerna = 23 Then
                            cadTerna = cadTerna.Replace("veintitres", "veintitrés")
                        ElseIf decenaTerna = 26 Then
                            cadTerna = cadTerna.Replace("veintiseis", "veintiséis")
                        End If


                    End If

                    'Completo miles y millones
                    Select Case iTerna
                        Case 3
                            If Numero < 2000000 Then
                                If Idioma = False Then
                                    cadTerna += " millón"
                                Else
                                    cadTerna += " million"
                                End If

                            Else
                                If Idioma = False Then
                                    cadTerna += " millones"
                                Else
                                    cadTerna += " millions"
                                End If

                            End If
                        Case 2, 4
                            If terna > 0 Then
                                If Idioma = False Then
                                    cadTerna += " mil"
                                Else
                                    cadTerna += " thousand"
                                End If
                            End If

                    End Select
                    Resultado.Insert(0, cadTerna)
                    Num = Int(Num / 1000)
                Loop
            End If

            'Se agregan los decimales si corresponde
            If Decimales > 0 Then
                Resultado.Append(" " + SeparadorDecimalSalida + " ")
                Dim EnteroDecimal As Int32 = Int(Math.Round((Numero - Int(Numero)) * Math.Pow(10, Decimales)))
                If ConvertirDecimales Then
                    Dim esMascaraDecimalDefault As Boolean = MascaraSalidaDecimal = MascaraSalidaDecimalDefault
                    Resultado.Append(Convertir(Convert.ToDecimal(EnteroDecimal), 0, Nothing, Nothing, EsMascaraNumerica, False,
                    False, (ApocoparUnoParteDecimal AndAlso Not EsMascaraNumerica), False) + " " + (IIf(EsMascaraNumerica, "", MascaraSalidaDecimal)))
                ElseIf EsMascaraNumerica Then
                    Resultado.Append(EnteroDecimal.ToString(MascaraSalidaDecimal))
                Else
                    Resultado.Append(EnteroDecimal.ToString() + " " + MascaraSalidaDecimal)
                End If
            End If
            'Se pone la primer letra en mayúscula si corresponde y se retorna el resultado
            If LetraCapital Then
                Return Resultado(1).ToString().ToUpper() + Resultado.ToString(2, Resultado.Length - 2)
            Else
                Return Resultado.ToString().Substring(1)
            End If
        End Function


        'No soporta Framework 4.5, habría que convertir todo.

        'Private Shared Function EnviarCorreo(ByVal destinatarios_ As List(Of String),
        '                                     ByVal subject_ As String,
        '                                     ByVal mensajeSimple_ As String,
        '                                     Optional ByVal mensajeEnvoltura_ As StringBuilder = Nothing,
        '                                     Optional ByVal espacioTrabajo_ As IEspacioTrabajo = Nothing) As Boolean

        '    Dim operacionesCorreo_ As IOperacionesCorreoElectronico

        '    operacionesCorreo_ = New OperacionesCorreoElectronico64


        '    Dim operacionesCatalogo_ = New OperacionesCatalogo

        '    Dim correo_ = New MensajeCorreoElectronico

        '    With operacionesCatalogo_

        '        .EspacioTrabajo = espacioTrabajo_

        '    End With

        '    With correo_

        '        .Too = destinatarios_

        '        .Subject = subject_

        '        If Not mensajeEnvoltura_ Is Nothing Then

        '            .Body = mensajeEnvoltura_.ToString

        '        Else

        '            'PENDIENTE DE IMPLEMENTAR

        '            .Body = "<!DOCTYPE html> <html lang=""en""> <head> <meta charset=""UTF-8""> <title>Recuperar Contraseña</title><link href=""https://fonts.googleapis.com/css?family=Roboto"" rel=""stylesheet""> <style> * { padding: 0; margin: 0; box-sizing: border-box; font-family: 'Roboto', sans-serif; } .container { width: 100%; } .wrapper { max-width: 400px; min-height: 500px; margin: auto; text-align: center; } .text { color: #424241; font-size: 14px; padding: 0px 10px; margin-top: 15px; margin-bottom: 50px; text-align: justify; } .text p { margin-bottom: 10px; } span.password { font-size: 18px; } .text p:nth-child(3) { font-size: 13px; } </style> </head> <body> <div class=""container""> <div class=""wrapper""> <!--<img src=""data:image/jpeg;base64," & _
        '                """ alt="""">--> <img src=""data:image/png;base64," & _
        '                "" & _
        '                "" & _
        '                """ alt=""""> <div class=""text""> " & _
        '                "<p>!Hola " & "NOMBRE USUARIOOO!!!!" & "¡ Lamentamos que perdieras tu contraseña, pero no te preocupes te proporcionamos una nueva :D</p> <p>Esta es tu nueva contraseña: <span class=""password"">" & _
        '                "<b>" & "CONTRASEÑA!!!" & "</b></span></p> <p>Te recordamos que una vez ingresando a nuestra extranet, en tu perfil puedes cambiar tu contraseña a una más de tu agrado.</p> </div> <img src=""data:image/png;base64," & _
        '                """ alt=""""> </div> </div></body> </html>"


        '        End If

        '    End With

        '    operacionesCorreo_.EnviarCorreo(operacionesCatalogo_,
        '                                    correo_,
        '                                    IOperacionesCorreoElectronico.Prioridades.Alta,
        '                                    IOperacionesCorreoElectronico.CantidadIntentosEnvio.Normal,
        '                                    IOperacionesCorreoElectronico.Modalidades.Individual)

        '    If operacionesCorreo_.Estatus.Status = TagWatcher.TypeStatus.Ok Then

        '        Return True

        '    Else

        '        Return False

        '    End If

        'End Function

    End Class

End Namespace
